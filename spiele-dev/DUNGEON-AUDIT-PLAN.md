...
Umgesetzt: B1-B10 + B12 + B13 + B15 + B16 (Wildnis-Integration: 🏰-Badge krieger + partner + Koloss-Quest mit win-Flag; verifiziert __nw.simStation → koloss nur bei win=1). B17 (Endgame: rebalancierte Score-Formel #63 Sieg-Min 212/Tod-Max 56, Lauf-Timer #68 mm:ss, Bestzeit in nd-best + 🏆-Feedback + Sieg-Arpeggio). B9/#30 (Floating-Joystick: springt zum Daumen in der linken Bildhälfte, Aktionsknöpfe rechts unberührt), B11 (Höhlen-Ambiente-Drone + Tropfen ab Start #21, Boss-Bass-Ostinato ab dem Aufstehen + Enrage-Tempo + Wut-Riff #23; verifiziert bossOn=true, 0 Fehler), B18 (Rematch-Knopf #67 → stationRematch lädt die Station frisch neu (Reward zählt regulär) + „← Wildnis", NG+/Meister-Modus #71 ?ng=1: Boss startet in Enrage, Score ×1.5, MEISTER-Label, „⚔️ Meister-Abstieg"-Knopf, NG-Läufe schreiben keine Bestzeit; verifiziert Smoke station=1 & ng=1 + Boss-Trigger), B14-teilweise (#57 Kristall-Regen ohne 10 setIntervals → frame-basierte fallGems-Liste, boss.falling hochgezogen → sinkt auch nach Sieg weiter). **BEWUSST VERSCHOBEN: B14/#54 (Licht-Pool) + #58 (InstancedMesh-Deko)** — die Audit-Prämisse „16 PointLights" gilt nicht mehr (Level vereinfacht → real ~8 Lichter: ~5-6 Fackeln + upLight/vLight/Spieler). Pooling auf 3 spart nur ~3 Lichter, gegen echtes Bruchrisiko in der Checkpoint-Fackel-Färbung (T4.lt-Kopplung) → Risiko > Nutzen. **Dungeon-Audit im Kern abgeschlossen.** OFFEN nur noch das optionale Licht-Pooling, falls je nötig. Anker vor Anwendung grep-verifizieren (Zeilennummern = Audit-Stand, driften durch Umsetzung).
