# spiele-dev/blender_landscape.py — prozedurale Neon-Hintergrund-Landschaft für neon-realm.html
# ----------------------------------------------------------------------------------------------
# Blender 4.0, headless:  blender -b -P spiele-dev/blender_landscape.py
# ⚠️ Der glTF-Exporter braucht numpy, das dem System-Blender fehlt. Einmalig:
#    python3.12 -m pip install --target /tmp/pymods numpy
#    PYTHONPATH=/tmp/pymods blender -b -P spiele-dev/blender_landscape.py
#
# Baut einen dekorativen RING um das Spielfeld (Spielfeld = Quadrat ±170, Spieler-Radius 168):
#   - Ring-Terrain (Annulus r 172..300) mit sanften Noise-Hügeln, innen tief abgesenkt (Skirt),
#     damit es unter der Spielfeld-Kante verschwindet und die Gameplay-Fläche unberührt bleibt.
#   - 11 markante Low-Poly-Berge am Horizont (r ~205..255).
#   - ~20 Neon-Kristall-Spitzen (Cyan/Pink, emissiv) an den Bergflanken (r ~188..240).
# 3 Materialien: rock (dunkles Violett/Blau) + crystal_cyan + crystal_pink (emissive Akzente).
# Export: models/landscape_realm.glb  (plain GLB, KEIN Draco — three.js r128!)
# Ziel: < 500 KB, < 15k Tris. Einheiten = Spieleinheiten, Ursprung = Weltmitte, Y-up im GLB.
import bpy, bmesh, math, random, os, sys
from mathutils import Vector, Matrix, Euler, noise

random.seed(4207)
OUT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models", "landscape_realm.glb")
...
def mk_mat(name, base, emis, strength=1.0, rough=0.85):
    ...
    # Blender 4.0: "Emission Color"/"Emission Strength" (ältere: "Emission")
...
# ---------- Ring-Terrain ----------
R0, R1 = 172.0, 300.0     # innen knapp an der Spielfeld-Kante (±170), aussen weit im Nebel
...
def ring_h(x, y, t):
    ...
    h = -3.0 * (1.0 - t) + t * 15.0 + (n1 * 9.0 + n2 * 3.0) * (0.25 + t)
    if t < 0.10:  # Skirt: Innenkante tief unter die Spielfeld-Kante ziehen
        h = -3.0 + (h + 3.0) * (t / 0.10)
    return h
...
for k in range(N_MTN):
    a = (k + 0.5) / N_MTN * 2.0 * math.pi + random.uniform(-0.18, 0.18)
    r = random.uniform(205.0, 255.0)
    x, y = r * math.cos(a), r * math.sin(a)
    base_r = random.uniform(17.0, 32.0)
    height = random.uniform(28.0, 56.0)
...
for k in range(14):
    ...
    big_r, big_h = random.uniform(2.6, 5.5), random.uniform(14.0, 32.0)
...
for ob in scn.collection.objects:
    if ob.type == "MESH":
        ...
        total_tris += tris
        print("MESH %-14s  polys=%5d  tris=%5d" % (ob.name, len(ob.data.polygons), tris))
print("TOTAL TRIS:", total_tris)
if total_tris > 15000:  # Decimate-Fallback (sollte hier nie greifen, Budget ~5k)
    for ob in scn.collection.objects:
        if ob.type == "MESH":
            mod = ob.modifiers.new("dec", "DECIMATE")
            mod.ratio = 14000.0 / total_tris
