...
mitSonden(quelle, {
  ...
  kriech: `function(ohne){var hh=12,dayA=Math.max(0,Math.sin((hh-5.75)/14*Math.PI)),px=Math.cos((hh-5.75)/14*Math.PI)*34,py=4+dayA*36,pz=18;
    var gl=renderer.getContext(),w=gl.drawingBufferWidth,h=gl.drawingBufferHeight,ref=null,R=[],x0=camTx,z0=camTz;
    ...
    for(var i=0;i<=10;i++){var d=i*0.02;window._sonneAufKamera(x0+d,z0+d*0.5,px,py,pz,ohne);
      ...
      if(!ref){ref=b;continue;}var n=0;for(var k=0;k<b.length;k+=4){if(Math.abs(b[k]-ref[k])+Math.abs(b[k+1]-ref[k+1])+Math.abs(b[k+2]-ref[k+2])>24)n++;}
      R.push(+(100*n/(w*h)).toFixed(3));ref=b;}
    still.forEach(function(o){o.visible=true;});var sc=sun.shadow.camera;
    return {prozent:R,texel:+((sc.right-sc.left)/sun.shadow.mapSize.x).toFixed(4)};}`
}, TMP)
...
console.log(`Texel ${mit.texel} m · Versatz 0…20 cm in 2-cm-Schritten, Anteil geaenderter Bildpunkte je Schritt (%):`)
console.log(`   mit Raster : ${mit.prozent.join(' · ')}`)
console.log(`   ohne Raster: ${ohne.prozent.join(' · ')}   ← Gegenprobe`)
