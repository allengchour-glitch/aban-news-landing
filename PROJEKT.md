...
> **🧠 ZWEITES GEHIRN installiert (User „2tes gehirn für selbstheilung"):** `tools/tool_brain.py` — QA/Selbstheilung speziell für die interaktiven Tools/Apps (alles mit data-aban-news-cta): prüft 0 Null-Bytes, JSON-LD-Gültigkeit, `node --check` des Logik-Scripts, canonical/viewport/title/description, Newsletter-Link + Hub-Rücklink. Score + `reports/TOOL-BRAIN-REPORT.md` + `automation/tool-brain-state.json`. `--fix` heilt Null-Bytes. Erstlauf: 134 Tools, Score 91,6/100, 0 hoch/mittel, 21 niedrig (meist „Hub-Rücklink fehlt" auf älteren Fremd-Tools → To-do). **Routine jetzt beide Gehirne:** `daily_improvement_scan.py --fix && tool_brain.py --fix`. Doku in `automation/BRAIN.md`. **YouTube:** fertiges Ankündigungs-Paket (Titel/Skript/Beschreibung/Tags/Community-Post) in `data/youtube-ankuendigung.md` — Upload nur durch User-Kanal (kein API-Zugang in Cloud-Session).
> **🩺 Loop-Iteration (Backlog→Bau, 3 P1-Ideen):** `promille-rechner.html` (Widmark-BAK + Abbauzeit, harter „nie fahren"-Disclaimer; getestet 500ml/5% ♂75kg = 0,38 ‰ Peak), `ssw-rechner.html` (SSW „X+Y" + Geburtstermin Naegele LMP+280±Zyklus, Trimester), `eisprung-rechner.html` (Eisprung = nächste Periode −14, fruchtbares Fenster; „keine Verhütung"). Alle Health/ehrliche Disclaimer, in Hub/Sitemap/Homepage. Lern-Sammler dedupte sie automatisch → Backlog 15 offen. Beide Gehirne 100/100. Hub ~140 echte Tools.
> **🚗 Loop-Iteration (Auto-Vergleich, P2):** `benzin-vs-elektro.html` (Energiekosten Benziner vs. E-Auto /100km & /Jahr; getestet 7L×1,80 vs 18kWh×0,30 @15'000km → 1890 vs 810, −1080/J) + `leasing-vs-kauf.html` (Leasing=Anzahlung+Raten vs. Kauf=Preis−Restwert+opt. Zins; getestet 24'600 vs 18'000). In Hub/Sitemap/Homepage, Backlog → 13 offen (nächste: Eigenmietwert). Hub ~142.
> **🏡 Loop-Iteration (Eigenmietwert + SEO-ROI):** `eigenmietwert-rechner.html` (CH; EMW ≈ 60–70 % Marktmiete, aus Miete ODER Wert×Rendite; Steuer-Effekt − Unterhalt/Zinsen; getestet 2500/Mt→19'500 EMW, 5'850 Steuer @30 %; im Geld-Center) + `seo-roi-rechner.html` (recherche-validiert: Traffic×CR×Auftragswert×Wiederkäufe×Marge − Budget → ROI/Break-even; getestet 1000@2 %/300/40 %−1500 → +60 % ROI). In Hub/Sitemap/Homepage. Backlog → 12 offen (nächste: Ferien-Anspruch). Hub ~144.
> **📊 KLICK-INSIGHTS (User wollte „sehen wo Leute klicken"; verstecktes Tracking ABGELEHNT — bricht „kein Tracking" auf 84 Seiten + DSGVO):** Stattdessen TRANSPARENTER, anonymer, cookieloser Klick-Zähler gebaut, **OFF by default** (wie `js/analytics.js`): `js/click-insights.js` (sendBeacon {Pfad,Label}, DNT-safe), `functions/api/click.js` (zählt in KV `CLICK_KV`, ohne Bindung No-Op), `functions/api/click-stats.js` (JSON, geschützt durch Env `CLICK_STATS_KEY`), `klick-statistik.html` (noindex Auswerter). **Bewusst NICHT auf Promise-Seiten verdrahtet** (online-tools/geld-center/index sagen „kein Tracking"). Aktivierung = User: KV binden + ON=true + Skript einbinden + Wortlaut/Datenschutz anpassen → Doku `automation/KLICK-INSIGHTS-SETUP.md` (inkl. Datenschutz-Textbaustein). Solange OFF: 0 Requests, Versprechen 1:1 gültig. **Empfohlen einfacher Weg bleibt: Cloudflare Web Analytics (Token in `js/analytics.js`) für „welche Seite bringt Besucher".**
> **🛠️ Hub-Links geheilt + 🇨🇭 Geld-Center:** 21 ältere Tool-Seiten hatten keinen `data-aban-hublink` → per Skript den selbst-zentrierenden Rücklink nach `</header>` eingefügt → **Gehirn 2 = 100/100**. NEU `schweizer-geld-center.html` — Landing-Page (CollectionPage+ItemList-JSON-LD) bündelt die 7 CH-Rechner + Freelancer-Cockpit als Karten-Grid + FAQ; Featured oben in Hub-Geld-Sektion, Sitemap 0.9, Homepage. Starker SEO-/Teilen-Anker („schweizer rechner"). Beide Gehirne 100/100.
...
> **🇨🇭 SCHWEIZER APPS (User „geile apps für Schweizer zum verkaufen"):** App-Serie für CH-Zielgruppe (zahlungskräftig, wenig gute Gratis-Tools). #1 LIVE: `hypothek-tragbarkeit.html` — „Kann ich mir das Haus leisten?" nach CH-Bankregeln (min 20% EK, max 80% Belehnung, 5% kalk. Zins + 1% Nebenkosten + Amortisation 2. Hypo über 66,67% in 15J, Tragbarkeit ≤33% Brutto). Ampel + nötiges Einkommen + Ist-Kosten bei echtem Zins; getestet 1M/200k EK/180k Eink → Quote 32,7% knapp tragbar. Featured Geld-Sektion, Sitemap 0.8, Homepage. **SERIE KOMPLETT (#2–5 live):** `saeule-3a-rechner.html` (Ersparnis=Einzahlung×Grenzsteuersatz, getestet 7056×30%=2117; + Endwert vorschüssig; Max 7056/35280 als Hinweis „jährlich prüfen"), `schweizer-lohnrechner.html` (Brutto→Netto, AHV/IV/EO 5,3% + ALV 1,1% bis Cap 148'200 exakt, BVG/NBU/Quellensteuer als User-Input, 13. Monat, Monats-/Jahres-Toggle; getestet 7000→6062/Mt), `mwst-rechner-schweiz.html` (8,1/2,6/3,8% auf-/herausrechnen), `feiertage-arbeitstage-kanton.html` (gregorianische Osterformel verifiziert 2024/26/27, 26-Kantons-Feiertagsmap, Feiertage einzeln umschaltbar, Arbeitstage-Zählung). Alle 🇨🇭, voll-lokal, ehrlich mit Disclaimern (keine erfundenen Zahlen), Featured in Hub/Sitemap/Homepage. **Lehre:** CH-Sozialsätze AHV 5,3%/ALV 1,1% sind offiziell-stabil → ok hartzucoden; BVG/Steuer/3a-Max variieren → User-Input bzw. „prüfen"-Hinweis. **#6–7 dazu:** `krankenkassen-franchise-rechner.html` (KVG: Eigenanteil=min(Kosten,Franchise)+10% max 700; 2 Varianten + Break-even; getestet 800 Kosten→Franchise 2500 günstiger) + `arbeitgeberkosten-rechner.html` (Brutto + AHV5,3+0,2/ALV1,1 fix + BVG/UVG/FAK Eingabe; getestet +16,2% Overhead). **CH-Serie = 7 Apps.** **#8–9 (autonomer Loop):** `mietzins-senkung-rechner.html` (Referenzzins-Senkung → −2,91 % je 0,25-Schritt VMWG; getestet 1,75→1,25 = −5,82 %, 2000→1883.60) + `pensionskasse-einkauf-rechner.html` (Einkauf×Grenzsteuersatz = Ersparnis, Netto-Kosten, Staffelung, 3-J-Sperrfrist-Hinweis; getestet 20000×30%=6000). Beide im Geld-Center + Hub/Sitemap/Homepage. **CH-Serie = 9 Apps + Geld-Center.**
> **🚀 GROSSPROJEKT — `freelancer-cockpit.html` (All-in-1-App, User „krasseste tool all in 1"):** Eine voll-lokale Single-Page-App mit Sidebar + 6 Modulen, localStorage-Persistenz (`aban_cockpit_v1`), Dark-Mode, Print: (1) Stundensatz-Kalkulator (Ziel+Kosten÷abrechenb. Jahresstd ×Aufschlag; getestet 72000/1100×1.1=72.00), (2) Angebots-/Rechnungs-Kalkulation (Positionen, MwSt, Netto/Brutto, druckbar), (3) Zeiterfassung (Start/Stop-Timer läuft über curMs(), bucht Einträge, Wert=Std×Satz), (4) Textbausteine (6 Vorlagen Angebot/Mahnung1+2/Absage/Termin/Auftragsbest., Platzhalter, Copy), (5) Skonto & Verzugszinsen (getestet 4.11), (6) Notizen+To-dos. Dashboard-KPIs ziehen aus allen Modulen. **Klar abgegrenzt vom Pro `business-cockpit.html`** (das macht echte Rechnungen+QR+Kunden) → Upsell-CTA. Featured in Hub (KI-Werkzeug-Sektion), Sitemap (0.8), Homepage. Voll-lokal, kein Server/Login/Tracking.
> **Geld-Fokus (User „will geld"):** (1) NL3 (`data/newsletter-versandbereit-3.md`) Verkaufs-Block geschärft — konkrete Pakete + Preise (Kit CHF12 / Vorlagen CHF19 / Prompts CHF19), „kein Abo" (Voice 10/10). (2) `shop.html` Hero-Trust-Strip ergänzt (ab CHF 12 · kein Abo · Sofort-Download · sicher per Stripe). Shop-Check-Verdikt: Shop ist conversion-stark (22 Produkte echte Stripe-Links, Bundle CHF79, FAQ, Trust) — **Engpass = Traffic, nicht die Seite.** (3) NEU `gratis-ki-prompts.html` — Lead-Magnet: 11 erprobte Copy-Prompts für Selbstständige (Angebot/Mail/Mahnung/Bewertung/Social/Preis-Einwand…), Kopier-Buttons, Newsletter-CTA + Shop-Upsell (Prompt-Bibliothek). In Hub (KI-Werkzeug-Sektion, prio), Sitemap (0.8), Homepage-Footer. **Geld-Rails verifiziert: Affiliate-Links 200, Shop 200, Kit-Download (34) live.** Ehrliche Lage: Kauf→Download-Flow funktioniert, es fehlen Leser → Lead-Magnet + Newsletter-Versand (User) sind der Hebel.
> **batch-48 (A interne Verlinkung + B Tools, Brain-Loop):** **A:** Tools-Hub `online-tools.html` neue Sektion „🏢 KI für deine Branche" → Branchen-Übersicht + 11 Top-Hubs (handwerker/aerzte/steuerberater/anwaelte/gastronomie/friseure/immobilienmakler/fotografen/coaches/elektriker/maler) verzahnt traffic-starken Hub mit Branchen-Cluster. (Branchen-Hub-Generator `add_branchen_funnel.py` + 368 Hubs bewusst NICHT angefasst = Parallel-Session.) **B:** spritverbrauch-rechner (realer L/100km aus getankt÷gefahren×100 + Kosten; distinkt von fahrtkosten=Trip-Kosten; getestet 45/600=7,5, Kosten/100km 13,43) + buchstabier-alphabet (NATO + Deutsch, Umlaut-Auflösung Ä→A-E, Ziffern, Kopieren). **Hub-Einträge ~176** (inkl. 12 Branchen-Links). **⚠️ KIT-DOWNLOAD-LEHRE (teuer):** #1308 (44 ZIPs mit NEUEM Salt) war ein FEHLER → per #revert zurückgenommen. FAKT: `DOWNLOAD_SALT` ist in Cloudflare-Projekt **`abannews`** seit Langem gesetzt (privat/verschlüsselt, UNBEKANNT), und die **originalen 34 committeten ZIPs passen dazu → 34 Downloads funktionieren bereits.** NIE die ZIPs mit einem neuen Salt neu bauen — das zerstört die Übereinstimmung! Es gibt ZWEI CF-Projekte (`abannews` OHNE Bindestrich = Live mit STRIPE_API_KEY; `aban-news` MIT Bindestrich = separat). Build-Minuten standen auf 3000/3000 (von den `*-radar`-Git-Projekten aufgebraucht) → git-Deploys gedrosselt; Direct-Upload (`wrangler pages deploy`) umgeht das. Offene Lücke: ~10 Katalog-Kits ohne ZIP-Datei — ließen sich nur mit dem ECHTEN (alten) Salt erzeugen. 3 Newsletter geprüft (Voice 10/10, alle Links ok) — versandbereit.
> **batch-47 (Brain-Loop):** Brain `--fix` 0/100. Tools: volumen-rechner (Würfel/Quader/Kugel/Zylinder/Kegel/Pyramide, Einheit cm/m/mm + Liter-Umrechnung, Formel; getestet Würfel 1000cm³=1L, Kugel r5 523.6, Zylinder 502.65) + geschwindigkeit-rechner (v/s/t je 2→1, km/h↔m/s, Zeit h/min/s; getestet 100km/1.5h=66.67km/h=18.52m/s, 60km/h·2h=120km, 100km@50=2h). **Hub ~160.**
> **batch-46 (Brain-Loop):** Brain `--fix` 0 Befunde/100. Tools: flaechen-rechner (Fläche+Umfang Rechteck/Quadrat/Kreis/Dreieck/Trapez/Parallelogramm, Formelanzeige, Defaults pro Form; getestet Rechteck 15/16, Kreis r2.5 19.63/15.71, Trapez 15) + wuerfel-roller (W4–W100, n Würfel + Modifikator, Summe, Höchstwert hervorgehoben, crypto-Zufall, Kopieren). **Hub ~158.**
> **batch-45 (mit Brain-Loop):** Brain-Scan `--fix` lief (0 Befunde, 100/100, 0 Dateien geändert → nur Auto-Artefakte verworfen). Tools: binaer-rechner (Bit-Arithmetik +/−/×/÷ + AND/OR/XOR/Shift, Eingabe bin/dez/hex → Ergebnis in 3 Systemen; getestet 1010+11=1101, 10&3=2, 10<<3=80) + kalorienbedarf-rechner (BMR+TDEE Mifflin-St-Jeor, Aktivitätsfaktor, Ab-/Zunehm-Richtwerte, ehrlicher „kein Arztersatz"; getestet M 80/180/35 BMR 1755 / TDEE 2413). **Hub ~157.**
> **batch-44:** lottozahlen-generator (Presets Lotto 6/49, Swiss 6/42, EuroMillions/Eurojackpot 5/50+2/12, eigene Regeln; crypto.getRandomValues unbiased, ohne Zurücklegen; ehrlicher „jede Kombi gleich wahrscheinlich"-Hinweis) + quersumme-rechner (Quersumme, digitale Wurzel iteriert, alternierende QS, Teilbarkeit 3/9/11; getestet 123456→21→3, altQS 3). **Hub ~155.** crypto-Zufall clientseitig OK (nur Workflow-Scripts dürfen kein Math.random/crypto).
> **batch-43:** primzahl-pruefer (Primzahl-Check per Probedivision bis √n, Primfaktorzerlegung, Teiler, nächste Primzahl; getestet 60 nicht-prim/2²×3×5, 97 prim) + mietrendite-rechner (Brutto-/Nettomietrendite, Kaufnebenkosten %, laufende Kosten, Kaufpreisfaktor, €/CHF-Umschalter; getestet 4,00 %/3,12 %/25×). **Hub ~153.** ⚠️ base64-konverter verworfen — `encoder.html` (Base64+URL+JSON) deckt es ab. **Slug-Check-Lehre erneut bestätigt: auch nach Synonym-Tools suchen (encoder vs base64-konverter).**
> **batch-42:** ggt-kgv-rechner (GGT+kgV mehrerer Zahlen, euklid. Algorithmus, Primfaktor-Weg; getestet 12/18/24→6 & 72) + zeitrechner (Modus Uhrzeit±Dauer mit Tagesüberlauf / Dauern summieren→Dezimalstunden; getestet 14:30+2:45=17:15, 1:30+0:45+2:15=4,5 h). **Hub ~151.** ⚠️ Trinkgeld-Rechner verworfen — `rechnung-teilen.html` deckt Aufteilen+Trinkgeld schon ab. **Außerdem: 3 fremde abannews-PRs für User gemergt** (#1298 Link-Checker geeicht, #1269 Swiss-Org-Schema, #1268 hreflang de-CH); #1111/#1270 hatten Konflikte → übersprungen; Cross-Projekt-LuxeStyle-PRs bewusst NICHT gemergt.
> **batch-41:** arbeitstage-rechner (Werktage Mo–Fr/+Sa zwischen 2 Daten, eigene Feiertage abziehen, Enddatum-inkl-Toggle, Stunden-Schätzung; getestet 5 Werktage, mit Feiertag 4) + schlaf-rechner (90-Min-Zyklen, Modus Aufstehen↔Bett, +15 Min Einschlafen, 3–6 Zyklen, 5–6 hervorgehoben; getestet 07:00→21:45/23:15). **Hub = 144 Tools.** Date-Math rein lokal mit `new Date(y,m-1,d)`.
> **batch-40:** notendurchschnitt-rechner (gewichteter Schnitt, DE 1–6 / CH 6–1, Zeilen add/remove, WHO-/Noten-Einordnung, getestet 2.67) + bmi-rechner (Gewicht÷Größe², WHO-Kategorien + Farb-Bar + Normalgewichts-kg-Bereich, ehrlicher „kein Arztersatz"-Disclaimer, getestet 23.1). **Hub = 142 Tools.** ⚠️ geplanten `prozentrechner.html` verworfen — `prozent-rechner.html` (mit Bindestrich) existiert schon (Prozent+Dreisatz). **Lehre erweitert: Slug-Check auch mit/ohne Bindestrich.** Link-Checker meldet 235 „defekte" `/go/...`-Links — ALLE aus den `*-kaufen-schweiz.html`-Kaufratgebern der Parallel-Session (dynamische Affiliate-Redirects), NICHT aus Tool-Seiten → nicht meine Baustelle.
...
- **Neu gebaut & live (alle mit WebApplication+FAQPage-Schema, in online-tools + sitemap, Brain 100/100):**
  ...
  wlan-qr (WIFI:-QR, **QR-Encoder aus qr-code.html wiederverwendet** via Python-Assembly, esc für ;,:") + passphrase-generator (merkbare DE-Wort-Passphrase, crypto, Entropie-Anzeige) #1191 ·
  ...
  fahrtkosten-rechner (Spritkosten Strecke×Verbrauch×Preis + Kilometerpauschale, Hin/Rück, pro Person) + backofen-temperatur (Gasstufe↔°C↔°F↔Umluft, Tabelle) #1210 ·
  stromkosten-rechner (Watt×Std×Tage×Preis oder kWh/Jahr → Tag/Monat/Jahr, getestet 51,10 €) + zeichen-limit-social (Live-Längen vs. X/Insta/LinkedIn/Meta-Title/Description, Ampel) #1224 ·
  sparziel-rechner (Zielbetrag→monatl. Sparrate, Annuität rückwärts, Startkapital, getestet 150,83 €) + inflations-rechner (Kaufkraft/Wertverlust, getestet 1000€→744€) [batch-35, 2026-06-24]. **Hub = 130+ Tools.**
