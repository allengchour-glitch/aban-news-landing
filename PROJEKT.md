...
> `tools/i18n_audit.py --fix` → verifizieren → committen → Memory. **Neu (Tools #7–#25):**
...
> Seitenverhältnis-Rechner, Einheiten-Umrechner, Marge-/Aufschlag-Rechner, Farbpaletten-Generator,
> Text-Werkzeug, Break-Even-Rechner, HTML-Entity-Encoder, Typo-Skala-Generator, Border-Radius-Generator,
> cubic-bezier-Generator — je viersprachig, in Sitemap (4/4), review-geprüft. **Das 2te Gehirn fand reale Bugs**
> (u.a. XSS via Dateiname, </script>-Injection im FAQ-Tool, WebP-Transparenzverlust, toBlob-Race,
> HIGH-Bug in der Datums-Differenz, `__proto__`-dedup-Falle, Marge≥100%-Guard). Per-Tool-Details in
> `SHARED-MEMORY.md` (LIVE-STAND, Tool #1–#25). **Tools NICHT zurückbauen.** Gesamt-Tool-Suite (mit
> früheren) ~25 review-grade Browser-Tools, alle viersprachig.
...
hat `/vergleich/` in EXCLUDE (Fremd-Toolpreise). **Offen (User, GitHub-Rate-Limit-Phasen):** PR #715 ist „ready". **QA 16.06.:** interne FR/IT-Links auf lokalisierte Siblings umgebogen (Sprach-Switcher ausgenommen), 0 FR/IT-Orphans, FR/IT-Startseiten verlinken den Tools-Hub. **i18n-Sync-Bot (16.06.):** `tools/i18n_audit.py` + `.github/workflows/i18n-sync.yml` halten DE/EN/FR/IT automatisch synchron (hreflang/Sitemap-Auto-Fix, Defekt-Gate).
