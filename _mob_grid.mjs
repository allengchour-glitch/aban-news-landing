...
let seq=0;
function curlGet(u){return new Promise(res=>{const f='/tmp/_gd'+(seq++);
...
const r=await p.evaluate(()=>{
 ...
 const cards=[...document.querySelectorAll('.product-grid .card, .card-gallery, li.product-grid__item, .product-card, [class*="product-grid"] > li')].slice(0,12);
 const out=cards.map(c=>{const rc=c.getBoundingClientRect();return {cls:(typeof c.className==='string'?c.className:'').slice(0,40),left:Math.round(rc.left),right:Math.round(rc.right),w:Math.round(rc.width),cut:rc.right>W+1};});
 const grid=document.querySelector('.product-grid,[class*="product-grid"]');
 let g=null; if(grid){const rc=grid.getBoundingClientRect();const cs=getComputedStyle(grid);
  g={left:Math.round(rc.left),right:Math.round(rc.right),w:Math.round(rc.width),gtc:cs.gridTemplateColumns,gap:cs.gap,pad:cs.padding,ovx:cs.overflowX,disp:cs.display};}
 return {W, docW:document.documentElement.scrollWidth, grid:g, cards:out, cut:out.filter(c=>c.cut).length};
});
