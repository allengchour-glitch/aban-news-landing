#!/usr/bin/env python3
"""Voice-Radar — static site generator (German, DACH-Fokus).

Reads the curated voice/transcription-tool database (data/anbieter.json) and an
...
du-Form — KI-Voice & Transkription ist hype-lastig, hier wird ehrlich verglichen.
...
SITE_NAME = "Voice-Radar"
BASE_URL = "https://voice.abannews.com"
LANG = "de"
...
UI = {
    "tagline": "KI-Voice- & Transkriptions-Tools, ehrlich verglichen — für den DACH-Raum",
    "all_tools": "← Alle Tools",
    "categories": "Kategorien",
    "focuses": "Schwerpunkt",
    "home_heading": "{n} Voice- & Transkriptions-Tools im Überblick",
    "provider": "Anbieter",
    "type": "Typ",
    "focus": "Schwerpunkt",
    "language": "Sprache",
    ...
    "best_in": "Beste Voice-Tools: {cat}",
    "focus_page": "Voice-Tools: {foc}",
    "privacy": "Datenschutz",
    "imprint": "Impressum",
    ...
    "honest": ("Ehrlich vorab: Ein Transkriptions-Tool nimmt dir das Zuhören nicht ganz ab. "
               "Bei Fachjargon, Dialekt oder schlechter Aufnahme musst du nachkorrigieren — "
               "und gerade bei deutscher Spracherkennung sind die Tools unterschiedlich gut. "
               "Lädst du Meeting-Audio hoch, verarbeitest du womöglich personenbezogene Daten: "
...
def faq_section(tool):
    ...
    # Deutsche Oberfläche / Spracherkennung (DACH-relevant)
    qa.append((f"Versteht {name} Deutsch und hat es eine deutsche Oberfläche?",
               {"ja": f"Ja, {name} unterstützt laut Anbieter deutsche Spracherkennung bzw. eine deutschsprachige Oberfläche.",
                "nein": f"Nein, {name} ist primär englischsprachig — deutsche Audio-Inhalte vor dem Einsatz testweise prüfen.",
    ...
    qa.append((f"Hostet {name} in der EU (DSGVO-relevant)?",
               {"ja": "Ja, laut Anbieter gibt es EU-Hosting bzw. EU-Datenverarbeitung — das erleichtert die DSGVO-Konformität, gerade beim Hochladen von Meeting-Audio.",
                "nein": "Nein, die Datenverarbeitung erfolgt überwiegend außerhalb der EU — Auftragsverarbeitung (AVV) und Drittlandtransfer (z. B. Standardvertragsklauseln) selbst prüfen, bevor du Audio mit personenbezogenen Daten hochlädst.",
...
def home_page(tools, categories, focuses, aff):
    ...
    body = (f'{site_schema()}{itemlist(tools)}'
            ...
            f'„{e(UI["eu_stock"])}“ ist der wichtigste DACH-Filter: DSGVO-näher, EU-Datenverarbeitung, weniger Drittland-Aufwand beim Audio-Upload.</p>\n'
...
LEGAL_DOMAIN = "voice.abannews.com"
...
beim Einsatz von Voice- und Transkriptions-Tools (z. B. Einwilligung der Aufgenommenen,
Auftragsverarbeitung nach Art. 28 DSGVO, ggf. Drittlandtransfer beim Audio-Upload) liegen bei dir.</p>
...
<h2>6. Hinweis zu Audio-Uploads bei den verglichenen Tools</h2>
<p>Die hier verglichenen Voice- und Transkriptions-Tools laufen bei den jeweiligen Anbietern,
nicht auf dieser Website. Lädst du dort Audio hoch (z. B. Meeting-Aufnahmen), verarbeitest du
...
def notfound_page():
    body = (f'<h1>🎙️ 404 — Seite nicht gefunden</h1>\n'
            f'<p class="meta">Diese Seite gibt es nicht. Zurück zur Übersicht aller Voice- & Transkriptions-Tools.</p>\n'
...
SEARCH_JS = """// Voice-Radar — client-side filter (no deps, no tracking)
(function () {
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Voice-Radar generator")
