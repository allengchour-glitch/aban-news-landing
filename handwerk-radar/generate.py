...
Verzeichnis fuer KI-Anbieter & -Freelancer im DACH-Raum. Geschaeftsmodell:
...
Datenintegritaet: Es werden KEINE Anbieter, Namen, Kontaktdaten oder Bewertungen
erfunden. data/anbieter.json startet leer bzw. mit klar markierten Platzhaltern
...
SITE_NAME = "Wärmepumpe- & Solar-Installateure DACH"
SITE_SHORT = "Heizung & Solar DACH"
TAGLINE = "Fachbetriebe für Wärmepumpe, Photovoltaik & Solar im DACH-Raum — sachlich gelistet, kein Hype"
BASE_URL = "https://handwerk.abannews.com"
...
LEISTUNGEN = ["Wärmepumpe", "Photovoltaik", "Solarthermie", "Stromspeicher"]
LEISTUNG_SLUG = {
...
LEISTUNG_EMOJI = {
    "Wärmepumpe": "♨️", "Photovoltaik": "☀️", "Solarthermie": "🌡️", "Stromspeicher": "🔋",
}
LEISTUNG_TEXT = {
    "Wärmepumpe": "Planung und Einbau von Luft-, Sole- und Wasser-Wärmepumpen — inkl. Heizlastberechnung.",
    "Photovoltaik": "PV-Anlagen für Dach und Fläche — Planung, Montage, Netzanschluss und Anmeldung.",
    "Solarthermie": "Solarkollektoren für Warmwasser und Heizungsunterstützung.",
    "Stromspeicher": "Batteriespeicher und Wallboxen — Auslegung, Einbau und Integration in die PV-Anlage.",
}
...
SUBMIT_BODY = (
    "Name der Anbieter/des Freelancers:\n"
    "Website:\n"
...
def itemlist_ld(entries) -> str:
    elems = [{"@type": "ListItem", "position": i + 1,
              "url": BASE_URL + page_path("anbieter", a["id"]),
...
<h2><a href="{e(page_path('anbieter', a['id']))}">{e(a['name'])}</a>{ph_badge}{feat_badge}</h2>
...
<h1>KI-Anbieter &amp; -Freelancer im DACH-Raum finden</h1>
...
<a class="cta ghost" href="{e(page_path('preise'))}">So funktioniert das Listing</a>
    ...
    desc = clip(f"Verzeichnis für KI-Anbieter und -Freelancer in DE, AT und CH — "
                f"nach Leistung und Land sortiert. Eintrag einreichen oder Featured-Listing buchen.")
    return page(title=f"{SITE_NAME} {date.today().year} — KI-Anbieter & -Freelancer finden",
...
def anbieter_page(a):
    is_ph = a.get("platzhalter")
    crumb = breadcrumb([(SITE_NAME, page_path("home")),
                        (a["name"], page_path("anbieter", a["id"]))])
    tagline = strip_redaktion(a.get("tagline") or "")
    ...
    if is_ph:
        ph_banner = ('<div class="note"><strong>Platzhalter-Eintrag.</strong> Dies ist ein '
                     'Struktur-Beispiel, keine echte Anbieter. Hier könnte dein Eintrag stehen — '
...
<a href="{e(page_path('preise'))}">Featured-Listing</a></div>"""
    ...
    return page(title=f"{a['name']} — {SITE_SHORT}", description=desc, body=body,
                canonical=BASE_URL + page_path("anbieter", a["id"]),
                kind="anbieter", slug=a["id"], noindex=is_ph)


def leistung_page(leistung, members):
    slug = LEISTUNG_SLUG[leistung]
    title = f"KI-{leistung}: Anbieter &amp; Freelancer im DACH-Raum"
    title_plain = f"KI-{leistung}: Anbieter & Freelancer im DACH-Raum"
    crumb = breadcrumb([(SITE_NAME, page_path("home")),
    ...
    return page(title=f"{title_plain} {date.today().year} — {SITE_SHORT}",
                description=clip(f"Anbieter und Freelancer für KI-{leistung} in DE, AT und CH. {intro}"),
...
def land_page(code, members):
    slug = code.lower()
    name = LAND_NAME[code]
    title = f"KI-Anbieter &amp; -Freelancer in {name}"
    title_plain = f"KI-Anbieter & -Freelancer in {name}"
    crumb = breadcrumb([(SITE_NAME, page_path("home")),
    ...
    body = (f'{crumb}{itemlist_ld(members)}'
            ...
            f'{submit_cta_block(prominent=False)}')
    return page(title=f"{title_plain} {date.today().year} — {SITE_SHORT}",
                description=clip(f"KI-Anbieter und -Freelancer aus {name} im DACH-Verzeichnis — "
...
<ul>
<li>Name der Anbieter / des Freelancers</li>
<li>Website</li>
...
LEGAL_DOMAIN = "anbieter.abannews.com"
...
def rss_feed(entries):
    ...
    for a in entries:
        if a.get("platzhalter"):
            continue
        link = BASE_URL + page_path("anbieter", a["id"])
...
def build(data_path: Path, out: Path) -> int:
    ...
    # Detailseiten
    for a in entries:
        of = out_file(out, "anbieter", a["id"])
        of.parent.mkdir(parents=True, exist_ok=True)
        of.write_text(anbieter_page(a), encoding="utf-8")
        pages += 1
    ...
    urls += [BASE_URL + page_path("anbieter", a["id"]) for a in entries if not a.get("platzhalter")]
