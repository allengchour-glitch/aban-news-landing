...
- **📧 KLAVIYO systemischer "Aban"-Absender-Rest (alle live DE-Flows via API geprueft):** Die ERSTE Mail jedes Flows wurde mit Absender=**"Aban"** angelegt (alte abannews), spaetere korrekt "LuxeStyle CH". Betroffen: **Vse76a** Abandoned E1+E2 (+reply-to=Gmail statt info@luxestyle.ch) & E3=DRAFT/Platzhalter-Betreff; **UKjAsV** Welcome-Serie E1 (= erste Mail JEDES neuen Abonnenten!). Korrekt: VbLQj6 Welcome, RnTvsY Post-Purchase (Order + Review T+10d). **Dubletten-Risiko:** 2 Welcome-Flows live (UKjAsV+VbLQj6, beide Added-to-List) + 2 Post-Purchase (RnTvsY DE + RNU6gh EN/US, beide Metric) → falls gleicher Trigger ohne Markt-Filter = Doppel-Mails. Alle Fixes = **NUR Klaviyo-UI** (kein update_flow in der API). Konsolidierte Klick-Liste an User.
...
- **🧹 Katalog-QA (Stichprobe, alles sauber):** Premium-Schmuck (107) · Drohnen&Kameras (7, Sneaker-Bug weg) · Im-Video-vorgestellt (7) — keine Fehlkategorisierung.
- **💸 Tote Collection wiederbelebt:** „🔥 Bestseller unter CHF 50" (nur 4 Produkte, beworben aber fast leer = Trust-Leck) → Smart-Collection **„💸 Lieblinge unter CHF 50"** mit Regel `Preis < 50` → **4 → 2904 Produkte** (echte Budget-Stöber-Seite, ehrlicher Name statt Fake-„Bestseller" bei 0 Verkäufen).
...
- **🤖 TikTok-BOT VOLL-SUITE / AUTOMOD MAX** (`automation/local/tiktok-bot.mjs`, Brave-CDP): **post** (stumm) · **analyze** (eigene Videos→`reports/tiktok-stats.csv`+Gehirn) · **engage** (Kommentar-Auto-Antwort via `chat/responder.mjs`, Spam-Filter, Cap 12, 8–20s anti-Block-Pausen, Ledger `reports/tiktok-engaged.txt`) · **delete** (dry-Default, `--go`, Cap 5, Top-5+`tiktok-keep.txt` geschützt, Ledger `tiktok-deleted.txt`) · **all/max** (analyze→post→engage). **Im `tiktok-cycle.ps1` voll automatisch:** jeder Lauf analyze+post+engage; **1×/Tag** Auto-Aufräumen (Verlierer <60 Views, Cap 3). Klick-Starter: `TIKTOK-MAX.bat`, `TIKTOK-ANALYSE.bat`, `TIKTOK-AUFRAEUMEN(-GO).bat`.
- **🤖 Upload-Bot** nimmt jetzt auch `mw-*` Meisterwerke + lädt **stumm** (TikTok-Trend-Sound-Regel). Regel `keine_dubletten_profil` im Gehirn (96 Regeln).
- **📈 IG/FB-Worker auf MAX:** Kommentar-Auto-Antwort Cap 12→18, 12 Posts × 40 Komm. gescannt, +Geschenk/Material-Themen (DE+EN). ⏳ live nach `wrangler deploy`.
- **🚀 WORKER SELBST-DEPLOY (maximum auto):** `automation/local/worker-autodeploy.ps1` — PC deployt den Worker automatisch bei Code-Änderung (Hash-Wächter), im `tiktok-cycle.ps1` verdrahtet. **Einmalig nötig: am PC `wrangler login` ODER `CLOUDFLARE_API_TOKEN` in luxe-secrets.ps1** (cfk_-Tokens waren ungültig). Danach 100% hands-off. Klick: `WORKER-DEPLOY-AUTO.bat`.
