...
- **🤖 TikTok-BOT VOLL-SUITE** (`automation/local/tiktok-bot.mjs`, Brave-CDP): **post** (delegiert, stumm) · **analyze** (eigene Videos scrapen→Report `reports/tiktok-stats.csv`+Gehirn) · **delete** (Trockenlauf-Default, `--go` nötig, Cap 5/Lauf, Top-5+`tiktok-keep.txt` geschützt, Ledger `reports/tiktok-deleted.txt`) · **all** (analyze→post). In `tiktok-cycle.ps1` verdrahtet (eingeloggte Analyse zusätzlich zu yt-dlp). Klick-Starter: `TIKTOK-ANALYSE.bat`, `TIKTOK-AUFRAEUMEN.bat` (dry), `TIKTOK-AUFRAEUMEN-GO.bat`.
