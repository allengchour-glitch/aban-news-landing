...
mhook () {
  ...
  if [ "$FMT" = "1x1" ]; then CW=1080; CH=1080; BIG=150; MID=46; SUB=34
  else CW=1080; CH=1920; BIG=180; MID=54; SUB=40; fi
  # intro card
  local IFC="[0:v]drawbox=x=80:y=(ih/2)-${BIG}*0.7:w=8:h=${BIG}*1.5:color=${GOLD}:t=fill,"
  IFC+="drawtext=fontfile=${SANS}:text='TOP 10':fontcolor=${INK}:fontsize=${BIG}:x=118:y=(h/2)-${BIG}*0.7:alpha='min(t/0.3\,1)',"
  IFC+="drawtext=fontfile=${SANS}:text='BESTSELLER 2026':fontcolor=${GOLD}:fontsize=${MID}:x=120:y=(h/2)+${BIG}*0.55:alpha='min(t/0.45\,1)'[v]"
  $FF -f lavfi -t 1.4 -i "color=c=${BG}:s=${CW}x${CH}:r=30" -filter_complex "$IFC" -map "[v]" -t 1.4 -r 30 \
      -c:v libx264 -pix_fmt yuv420p -crf 18 -preset veryfast "$W/MH${FMT}_intro.mp4"
  # outro card
  ...
  $FF -f lavfi -t 1.8 -i "color=c=${BG}:s=${CW}x${CH}:r=30" -filter_complex "$OFC" -map "[v]" -t 1.8 -r 30 \
      -c:v libx264 -pix_fmt yuv420p -crf 18 -preset veryfast "$W/MH${FMT}_outro.mp4"
  # assemble: intro + 10 trimmed modern segs + outro, fast slide
  local inputs="-i $W/MH${FMT}_intro.mp4"; local durs=( 1.4 )
  for f in "${files[@]}"; do inputs+=" -t $SEGT -i $W/M${FMT}_seg_${f}.mp4"; durs+=( $SEGT ); done
  inputs+=" -i $W/MH${FMT}_outro.mp4"; durs+=( 1.8 )
  ...
  $FF $inputs -filter_complex "$fc" -map "[vout]" -r 30 -c:v libx264 -pix_fmt yuv420p -crf 19 -preset medium \
      -movflags +faststart "$OUT/luxestyle_modernhook_${FMT}.mp4"
