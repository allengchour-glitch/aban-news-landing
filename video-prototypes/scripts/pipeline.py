...
SR_BASE=16000; SR_CART=19520          # pitch-up factor 1.22 (cartoon)
PITCH=SR_CART/SR_BASE
...
for k in keys:
    ...
    sh(["bash","-lc",f'cat /tmp/sc_{k}.txt | python3 -m piper -m {VOICE} --length_scale 0.95 --noise_w 0.9 -f /tmp/{k}_base.wav'])
    # cartoon pitch + warm EQ + de-harsh + compress + upsample
    eq=(f"asetrate={SR_CART},aresample={SR_BASE},highpass=f=110,lowpass=f=7200,"
        f"equalizer=f=350:t=q:w=1.2:g=2.5,equalizer=f=5000:t=q:w=2:g=-3,"
        f"acompressor=ratio=3:threshold=-18dB,dynaudnorm=f=200,aresample=24000")
