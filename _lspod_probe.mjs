...
const r=await p.evaluate(()=>({anchor: !!document.getElementById('lspod-start'), designer: !!document.querySelector('.lspod-designer'), cta: (document.querySelector('.lspod-designer')||{}).innerText? [...document.querySelectorAll('.lspod-designer button')].map(b=>b.innerText.trim()).filter(Boolean):[], nativeATC: document.querySelectorAll('form[action*="/cart/add"] button[name=add]').length, payBtn: document.querySelectorAll('.shopify-payment-button').length, formVariant: (document.querySelector('form[action*="/cart/add"] [name=id]')||{}).value })); console.log('PRUEF '+JSON.stringify(r)); await p.screenshot({ path:OUT });
