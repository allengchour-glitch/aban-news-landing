#!/usr/bin/env python3
"""Buchhaltung-Radar — static site generator (German, DACH-Fokus).

Reads the curated bookkeeping/invoicing-tool database (data/anbieter.json)
and an affiliate-link mapping (affiliate.json) and renders a small, SEO-oriented
static site:
...
SITE_NAME = "Buchhaltung-Radar"
BASE_URL = "https://buchhaltung.abannews.com"
LANG = "de"
...
UI = {
    "tagline": "KI-Buchhaltungs- & Rechnungs-Tools, ehrlich verglichen — für Selbstständige & KMU im DACH-Raum",
    "all_tools": "← Alle Tools",
    "categories": "Kategorien",
    "focuses": "Schwerpunkt",
    "home_heading": "{n} Buchhaltungs- & Rechnungs-Tools im Überblick",
    "provider": "Anbieter",
    "type": "Typ",
    "focus": "Schwerpunkt",
    "language": "Sprache",
    ...
    "best_in": "Beste Buchhaltungs-Tools: {cat}",
    "focus_page": "Buchhaltungs-Tools: {foc}",
    "privacy": "Datenschutz",
    "imprint": "Impressum",
    ...
    "honest": ("Ehrlich vorab: Eine Buchhaltungs-Software ersetzt keine saubere Buchführung "
               "und keine Steuerberatung. GoBD-Konformität, korrekte UStVA und ein sauberer "
               "DATEV-Export hängen davon ab, wie du das Tool nutzt und wie ihr es mit "
...
FAVICON_SVG = (
    ...
    '<rect x="16" y="12" width="32" height="40" rx="4" fill="none" stroke="#fff" '
    'stroke-width="3" stroke-linejoin="round"/>'
    '<path d="M22 24 H42 M22 32 H42 M22 40 H34" fill="none" stroke="#fff" stroke-width="3" '
...
def faq_section(tool):
    ...
    qa.append((f"Hat {name} eine deutsche Oberfläche?",
               {"ja": f"Ja, {name} bietet laut Anbieter eine deutschsprachige Oberfläche bzw. Support.",
    ...
    qa.append((f"Hostet {name} in der EU (DSGVO-relevant)?",
               {"ja": "Ja, laut Anbieter gibt es EU-Hosting bzw. EU-Datenverarbeitung — das erleichtert die DSGVO-Konformität. GoBD-Pflichten zur revisionssicheren Aufbewahrung musst du trotzdem selbst einhalten.",
                "nein": "Nein, die Datenverarbeitung erfolgt überwiegend außerhalb der EU — Auftragsverarbeitung und Drittlandtransfer (z. B. Standardvertragsklauseln) selbst prüfen.",
                "—": "Teilweise/abhängig vom Tarif — vor dem Start beim Anbieter und in dessen DPA prüfen."}[bool_str(tool.get("eu_lager"))]))
    # Integrationen
...
def home_page(tools, categories, focuses, aff):
    ...
    body = (f'{site_schema()}{itemlist(tools)}'
            ...
            f'„{e(UI["eu_stock"])}“ ist der wichtigste DACH-Filter: DSGVO-näher, EU-Datenverarbeitung, weniger Drittland-Aufwand.</p>\n'
...
LEGAL_DOMAIN = "buchhaltung.abannews.com"
...
<h2>Keine Rechts-/Steuerberatung</h2>
<p>Dieser Radar vergleicht Werkzeuge und ersetzt keine Rechts- oder Steuerberatung. Pflichten
in der Buchhaltung (z. B. GoBD-konforme, revisionssichere Aufbewahrung, korrekte
Umsatzsteuer-Voranmeldung über ELSTER, Auftragsverarbeitung nach Art. 28 DSGVO, Abstimmung
mit dem Steuerbüro) liegen beim Unternehmen.</p>
...
def notfound_page():
    body = (f'<h1>🧮 404 — Seite nicht gefunden</h1>\n'
            f'<p class="meta">Diese Seite gibt es nicht. Zurück zur Übersicht aller Buchhaltungs- & Rechnungs-Tools.</p>\n'
...
SEARCH_JS = """// Buchhaltung-Radar — client-side filter (no deps, no tracking)
(function () {
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Buchhaltung-Radar generator")
