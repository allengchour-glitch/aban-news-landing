#!/usr/bin/env python3
"""Video-Radar — static site generator (German, DACH-Fokus).

Reads the curated KI-video-tool database (data/anbieter.json) and an
...
du-Form — KI-Video ist hype-lastig, hier wird ehrlich verglichen.
...
SITE_NAME = "Video-Radar"
BASE_URL = "https://video.abannews.com"
LANG = "de"
...
UI = {
    "tagline": "KI-Video-Tools (Generierung, Avatare, Schnitt), ehrlich verglichen — für den DACH-Raum",
    "all_tools": "← Alle Tools",
    "categories": "Kategorien",
    "focuses": "Schwerpunkt",
    "home_heading": "{n} KI-Video-Tools im Überblick",
    "provider": "Anbieter",
    "type": "Typ",
    "focus": "Schwerpunkt",
    "language": "Sprache",
    ...
    "best_in": "Beste KI-Video-Tools: {cat}",
    "focus_page": "KI-Video-Tools: {foc}",
    "privacy": "Datenschutz",
    "imprint": "Impressum",
    ...
    "honest": ("Ehrlich vorab: KI-Video-Tools liefern selten den fertigen Clip auf Knopfdruck. "
               "Avatare wirken manchmal künstlich, Text-zu-Video braucht oft mehrere Anläufe, "
               "und Schnitt, Untertitel und Übersetzung musst du nachkorrigieren. "
               "Lädst du Material mit erkennbaren Personen hoch, verarbeitest du womöglich personenbezogene Daten: "
               "Einwilligung, Auftragsverarbeitung (AVV) und Hosting-Standort musst du selbst "
...
def faq_section(tool):
    ...
    qa.append((f"Hostet {name} in der EU (DSGVO-relevant)?",
               {"ja": "Ja, laut Anbieter gibt es EU-Hosting bzw. EU-Datenverarbeitung — das erleichtert die DSGVO-Konformität, gerade beim Hochladen von Video-/Bildmaterial.",
                "nein": "Nein, die Datenverarbeitung erfolgt überwiegend außerhalb der EU — Auftragsverarbeitung (AVV) und Drittlandtransfer (z. B. Standardvertragsklauseln) selbst prüfen, bevor du Video-/Bildmaterial mit personenbezogenen Daten hochlädst.",
...
def home_page(tools, categories, focuses, aff):
    ...
    body = (f'{site_schema()}{itemlist(tools)}'
            ...
            f'„{e(UI["eu_stock"])}“ ist der wichtigste DACH-Filter: DSGVO-näher, EU-Datenverarbeitung, weniger Drittland-Aufwand beim Upload von Video-/Bildmaterial.</p>\n'
...
LEGAL_DOMAIN = "video.abannews.com"
...
beim Einsatz von KI-Video-Tools (z. B. Einwilligung abgebildeter Personen,
...
<h2>6. Hinweis zu Uploads bei den verglichenen Tools</h2>
<p>Die hier verglichenen KI-Video-Tools laufen bei den jeweiligen Anbietern,
nicht auf dieser Website. Lädst du dort Video-/Bildmaterial hoch (z. B. mit erkennbaren Personen), verarbeitest du
womöglich personenbezogene Daten Dritter. Hosting-Standort, Auftragsverarbeitung (AVV) und
...
def notfound_page():
    body = (f'<h1>🎬 404 — Seite nicht gefunden</h1>\n'
            f'<p class="meta">Diese Seite gibt es nicht. Zurück zur Übersicht aller KI-Video-Tools.</p>\n'
...
SEARCH_JS = """// Video-Radar — client-side filter (no deps, no tracking)
(function () {
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Video-Radar generator")
