...
> Eigenes WM-Extra: **„⚽ WM-Trikot selbst gestalten – mit Name & Nummer"** (`/products/wm-trikot-selbst-gestalten`, Produkt-ID 15433948070273, ACTIVE, 5 Grössen S–2XL à CHF 34.90, alle 7 Kanäle). Nutzt den vorhandenen **`pod/designer.js`-Editor** (Text-Ebenen = Name + Nummer, Farbe/Grösse frei). **Jersey-Blanks** (Vorne CH-Kreuz/„SCHWIIZ", Hinten „DEIN NAME"+„10"-Platzhalter) via `automation/render_wm_trikot.py` (Pillow) → `pod/templates/wm-trikot-{front,back}.png`, auf **Shopify-CDN** hochgeladen (Feature-Branch wird NICHT über abannews.com ausgeliefert → CDN nötig; `stagedUploadsCreate`+`fileCreate`, Skript `/tmp/upload_blanks.mjs`). Tags `wm-2026`+`sg-bekleidung`+`printful_personalized_product` → in WM-Collection + Teardown + SG-Editor-Kategorie. **⚠️ Offen wie bei allen SG-Produkten: POD-Fulfillment-Mapping (Printful/Gelato-Variante)** — PC-Claude/User.
