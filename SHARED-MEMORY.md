...
- **Tool #56 (23.06.) via 2tes-Gehirn:** Römische-Zahlen-Umrechner (eine Eingabe, Auto-Richtung: Ziffern→römisch via toRoman, Buchstaben→Zahl via fromRoman; Bereich 1–3999; Copy mit Guard). **Kanonische Validierung** durch Re-Encoding (toRoman(n)===s) → lehnt IIII/VV/IC/MMMM ab. Review (node, erschöpfend): **alle 3999 Werte round-trippen** (toRoman→fromRoman=Original); alle ungültigen Formen (IIII/VV/IC/IL/XXXX/VX/MMMM) →null; 0/4000/„00"→Fehler; parseInt radix 10 (kein Oktal-Bug bei „007"→VII); 5000×„M" kein Hang (Range-Guard greift sofort); alles via textContent (kein innerHTML, dir nur numerisch/[IVXLCDM]) → kein XSS; Copy-Guard via res.className; 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — toRoman/fromRoman/PAIRS/Regex byte-identisch; nur Richtungs-/Fehler-/Flash-Strings je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 56 (Site: 911 Familien / 3088 Seiten).
- **Tool #55 (23.06.) via 2tes-Gehirn:** Skonto-Rechner (Rechnungsbetrag + Skonto % + Skontofrist + Zahlungsziel → Skontobetrag, Zahlbetrag, **effektiver Jahreszins** Skonto/(100−Skonto)×360/(Ziel−Frist)). Finanz, Pendant zu Rechnung/MwSt (cross-linked). **Installierter Bot fing beim Bauen einen realen Syntaxfehler ab** (gerades ASCII-`\"` nach „verdient\" beendete den JS-String → auf curly „…“ gefixt) — wieder ein Beleg, dass das 2te Gehirn meine eigenen Fehler vor dem Review catcht. Review (Formel+Edge): 1000/2%/10/30→20,00/980,00/36,7%; 3%/14/30→69,6%; Guards für skonto≤0 und days≤0 (keine Division durch 0/neg), skonto auf 99 geclamped (Nenner≥1); NaN→0; alle Quotes curly verifiziert; alles textContent/Intl → kein XSS; 0 weitere Bugs; bot 0/0; html-validate exit 0. Viersprachig — num/build/Formel byte-identisch; nur Intl-Locale + 3 Hint-Strings/Labels je Sprache (Hints ohne eingebettete Quotes → kein Trap). 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 55 (Site: 910 Familien / 3084 Seiten).
- **Tool #54 (23.06.) via 2tes-Gehirn:** Stoppuhr online (Count-up mit Start/Stopp/Weiter, Runden/Laps-Tabelle, Reset, Hundertstelsekunden). Distinkt vom Pomodoro-Countdown (zählt hoch + Laps). **Drift-fest:** acc + (running?Date.now()-startAt:0). Review (Zustandsautomat): Pausierzeit korrekt ausgeschlossen (acc=elapsed() bei Stopp, startAt neu bei Weiter); **kein Doppel-Interval** (Interval nur im else-Zweig, geclear't bei Stopp/Reset); fmt verifiziert (0→00:00.00, 65240→01:05.24, 3661000→1:01:01.00, cs 0–99); clock.firstChild = Text-Node (kein Whitespace davor) korrekt mutiert, cs separat; Lap-Delta (erster=Gesamt), prepend; Button-States ohne Desync; alles textContent → kein XSS; aria-live=off bewusst (Zentisekunden würden Screenreader fluten); 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — JS byte-identisch außer Start/Stopp/Weiter-Strings. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 54 (Site: 909 Familien / 3080 Seiten).
- **🎬 YouTube-Tool-Serie (User-Auftrag 23.06. „lern youtube tool installieren, dann recherchieren ganz viele, dann umsetzen auto"):** WebSearch-Recherche der gefragtesten **keyless** YouTube-Tools (YtToolBox/HCODX/ThumbnailDown) → Backlog: #50 Thumbnail-Downloader ✅, #51 Embed-Code-Generator, #52 Timestamp-Link-Generator, #53 Einnahmen-/CPM-Rechner. Alle rein client-seitig, KEINE API/Key, viersprachig, 2tes-Gehirn-Gate. Wird autonom Tool für Tool umgesetzt.
- **Tool #53 (23.06.):** YouTube-Einnahmen-/RPM-Rechner (monatl. Aufrufe + RPM-Spanne niedrig/hoch → geschätzte Einnahmen/Monat & /Jahr als Spanne, Intl-Currency EUR). **Schließt die YouTube-Serie ab (#50–#53).** Reine Mathematik, keine ID/kein Netz. Verifiziert (node): 100k Aufrufe @ RPM 1–4 = 100–400 €/Monat, 1200–4800 €/Jahr; lo>hi-Swap korrekt; NaN→0. Alles via textContent (Intl) → kein XSS; bot 0/0; html-validate. Ehrliche Disclaimer (Schätzung, 45 % YouTube-Anteil im RPM). Viersprachig — num/build byte-identisch; nur Intl-Locale + Labels. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 53 (Site: 908 Familien / 3076 Seiten). **YouTube-Serie komplett: Thumbnail (#50), Embed (#51), Timestamp (#52), Einnahmen (#53).**
- **Tool #52 (23.06.):** YouTube-Timestamp-Link-Generator (Link/ID + Std/Min/Sek → youtu.be?t=N + watch?v=…&t=Ns, beide kopierbar/klickbar). Verifiziert (node): 1h1m30s→3690s, URLs korrekt, intv-Clamps (NaN→0, >max→max), malicious→"" (gleicher validierter ID+Integer-Pfad wie #50/#51, bereits 2× tief reviewt → Schnellcheck statt voller Agent). Alles via textContent/.href zu validierten youtube-URLs → kein XSS; bot 0/0; html-validate. Viersprachig — extractId/intv/row byte-identisch; nur Fehlertext/Labels/Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 52 (Site: 907 Familien / 3072 Seiten). Backlog YouTube: #53 Einnahmen-/CPM-Rechner (letztes).
- **Tool #51 (23.06.) via 2tes-Gehirn:** YouTube-Embed-Code-Generator (Link/ID + Checkboxen Datenschutz-Modus/rel/autoplay/mute/loop/controls/responsiv + Start/Breite/Höhe → iframe-Code + Live-Vorschau via echtem iframe; Copy mit Guard). DeveloperApplication, DSGVO-Argument (youtube-nocookie). Review (ID+XSS=Kerncheck): ID strukturell `[A-Za-z0-9_-]{11}` → kein Scheme/Host-Ausbruch; src = fixer host + /embed/ + id + literale Flags + Integer-start → kein Userfreitext im iframe-src; **Code-String NUR via code.textContent** (nicht innerHTML) → `<iframe>`-Markup inert, `dQw…"></iframe><script>` ergibt nur saubere ID; intv() clamps (kein NaN), W/H disabled-Sync bei responsive; clr(preview) verhindert iframe-Akkumulation; loop→playlist=id; Default-src verifiziert `…nocookie.com/embed/ID?rel=0`; 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — extractId/intv/srcFor/codeFor byte-identisch; nur Fehlertext/ID-Label/Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 51 (Site: 906 Familien / 3068 Seiten). Backlog YouTube: #52 Timestamp-Link, #53 Einnahmen-/CPM-Rechner.
- **Tool #50 (23.06.) via 2tes-Gehirn:** YouTube-Thumbnail-Downloader (Link/ID einfügen → 5 Auflösungen maxres/sd/hq/mq/default als Vorschau-Cards mit „Öffnen"+„URL kopieren"; img-onerror→„nicht verfügbar"). Bilder direkt vom YouTube-CDN (i.ytimg.com), Eingabe verlässt Browser nie. **Ehrlich kommuniziert:** kein clientseitiger Direkt-Download (Cross-Origin) → „Öffnen"+Rechtsklick-Speichern. Review (ID-Extraktion = Kerncheck): extractId deckt watch?v=/youtu.be/shorts/embed/live/v/ + list-vor-v + nackte 11-Zeichen-ID; **ID strukturell garantiert `[A-Za-z0-9_-]{11}`** → kann nie aus i.ytimg-Host ausbrechen (kein /,.,:); alles via textContent/.src/.href (kein innerHTML) → `evil.com?v=`-Fall harmlos, kein XSS; onerror-Replace ohne Loop (parentNode-Guard); clr() leert Grid; 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — extractId/Regex/card/copyText byte-identisch; nur RES-Labels + Fehlertext + Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 50 (Site: 905 Familien / 3064 Seiten).
- **Tool #49 (23.06.) via 2tes-Gehirn:** Passwort-Stärke-Prüfer (Entropie-Bits, geschätzte Knackzeit-Bucket, Stärke-Balken, Checkliste Länge/Klasse, Show/Hide-Toggle). Pendant zum Passwort-Generator (cross-linked), SecurityApplication, hohes Suchvolumen, **Privacy-First** (alles lokal). Review (Privacy = wichtigster Check): Passwort verlässt NIE den Browser — kein fetch/XHR/Beacon/Form/URL/Storage/Cookie/Analytics; announce.js (alle 73 Zeilen geprüft) macht 0 Netz-Calls + berührt #pw nie; spellcheck=off. Mathe verifiziert: „password"=37,6 Bit→„Sekunden", „P@ssw0rd!"=59,1 Bit→„Monate" (log-space timeBucket, kein Overflow bei 1000 Zeichen/6600 Bit); leer→0/„sofort". Selbst gefangen+gefixt: CSS `input[type=text]` brach beim Toggle zu type=password → auf `#pw` umgestellt (Styling bleibt in beiden States). Passwort nur via value/textContent/style.width → `<img onerror>` inert, kein XSS; 0 weitere Bugs; bot 0/0; html-validate exit 0. Viersprachig — pool/timeBucket/update/COL/Schwellen byte-identisch; nur VERD/TIME/LABELS + Toggle-Strings je Sprache. 4 Hubs, Sitemap 4/4. **Lehre: type-abhängige CSS-Selektoren brechen bei Show/Hide-Toggles — per id stylen.** Tool-Suite jetzt 49 (Site: 904 Familien / 3060 Seiten).
- **Tool #48 (23.06.) via 2tes-Gehirn:** Pomodoro-/Fokus-Timer (Presets 25/5/15 + eigene Zeit, Start/Pause/Reset, Signalton via Web-Audio-Oszillator hinter Checkbox, Restzeit im Tab-Titel). Erster interaktiver Timer der Suite (productivity). **Drift-fester Pattern:** absoluter `endAt`-Timestamp + setInterval(250) rechnet remain neu → übersteht Tab-Throttling. Review (timer-fokussiert): **kein Doppel-Interval möglich** (start() pausiert wenn running, alle Restarts via stop()→setTime); Pause/Resume bewahrt Restzeit (stop() lässt remain unberührt, endAt neu); tick() clear't bei 0; Tab-Titel restauriert auf baseTitle bei Pause/Ende; 0:00-Start stoppt nach 1 Tick; beep() guarded+try/catch (autoplay-suspend = Browser-Limit, kein Bug); alles textContent/title via fmt() → kein XSS; 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — JS byte-identisch außer Start/Pause-Button-Strings. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 48 (Site: 903 Familien / 3056 Seiten).
- **Tool #47 (23.06.) via 2tes-Gehirn:** Zinseszins-/Sparrechner (Startkapital + monatliche Sparrate + Zinssatz p.a. + Laufzeit → Endkapital, Eingezahlt, Zinsertrag + Jahres-Tabelle Einzahlungen/Zinsen/Stand). Pendant zum Kredit-Rechner (FinanceApplication), hohes Suchvolumen. Review (in node gegen Closed-Form FV=P·(1+i)ⁿ+dep·((1+i)ⁿ−1)/i): P=5000/dep=200/5%/15J = **64026,31** Endkapital (auf den Cent identisch), 41000 eingezahlt, 23026,31 Zinsen; i=0-Fall (P+dep·n, keine Division durch 0), dep=0-Fall (reiner Zinseszins); Jahres-Summen rekonzilieren (Einzahlungen-Spalte exkl. Startkapital = korrekt), Teiljahr „*", 960-Iter-Cap kein Freeze; alle Ausgaben textContent/Intl → kein XSS; 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — num/build/Schleife byte-identisch; nur Intl-Locale + Labels je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 47 (Site: 902 Familien / 3052 Seiten).
- **Tool #46 (23.06.) via 2tes-Gehirn:** Text-Umwandler Gross-/Kleinschreibung (GROSS/klein/Titel-/Satzschreibung + Gross-klein-tauschen, Live-Zähler Zeichen/ohne-Leerz./Wörter/Zeilen, Copy mit „nichts zu kopieren"-Guard). Hohes Suchvolumen („text in grossbuchstaben umwandeln"). Review (in node ausgeführt): title→„Aban News: 5 Minuten Ki…", sentence→„Aban news: … Jeden werktag.", ß→„SS", Umlaute/Akzente korrekt, invert lässt Ziffern/Emoji unberührt; **kein ReDoS** (100k+100k in 33 ms), title-Charклass valide; alles via textarea.value/textContent → kein XSS auch bei `<script>`-Input; Clipboard guarded; 0 Bugs (1 kosmetischer Klammer-Hinweis, kein Defekt); bot 0/0; html-validate exit 0. Viersprachig — up/lo/title/sentence/invert/transform byte-identisch; nur Zähler-Labels + Flash + Placeholder je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 46 (Site: 901 Familien / 3048 Seiten).
- **Tool #45 (23.06.) via 2tes-Gehirn:** Kredit-/Darlehensrechner (Annuität: Betrag/Zinssatz p.a./Laufzeit → Monatsrate, Gesamtzinsen, Gesamtkosten + Jahres-Tilgungsplan, letzte Teiljahr-Zeile mit „*"). Ergänzt das Finanz-Trio (Finanz-/MwSt-/Marge-Rechner). Review (Mathe-fokussiert): unabhängig nachgerechnet — P=20000/4.5%/5J = **372,86/Monat**, 2371,62 Zinsen; i===0-Zweig (P/n) korrekt; Restschuld endet exakt 0; `pp>bal`-Guard verhindert Überzahlung/Negativsaldo; 600-Iter-Cap kein Freeze; alle Ausgaben via textContent (Intl) → kein XSS; 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — num/build/Tilgungsschleife byte-identisch; nur Intl-Locale + Labels je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 45 (Site: 900 Familien / 3044 Seiten).
- **Tool #44 (23.06.) via 2tes-Gehirn:** E-Mail-Signatur-Generator (Name/Rolle/Firma/Tel/E-Mail/Web/Slogan + Akzentfarbe → tabellenbasierte HTML-Signatur; Live-Vorschau + 2 Copy-Wege: „HTML kopieren" Quellcode und „Signatur kopieren" als rich `ClipboardItem` text/html+text/plain mit copyText-Fallback). Für Selbstständige (verlinkt Rechnung+QR). Review (XSS-fokussiert): preview.innerHTML escape-first — **jeder** Userwert via esc(), Akzentfarbe nur via `/^#[0-9a-f]{6}$/`-Allowlist mit Fallback (kein Style-Breakout), safeUrl rejectet `[\s"'<>\\]`+verlangt http(s), mailto nur bei gültigem Regex → 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — esc/safeUrl/accentColor/build/copyRich byte-identisch; nur Labels + „Tel:" + 2 Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 44 (Site: 899 Familien / 3040 Seiten).
- **Tool #43 (22.06.) via 2tes-Gehirn:** Breadcrumb-Schema-Generator (Pfad-Zeilen Name+URL → BreadcrumbList-JSON-LD, Auto-Position, letzte Stufe URL-optional; `</script>`/`<`-Escaping). Komplettiert das SEO-Schema-Trio (FAQ + LocalBusiness + Breadcrumb). Review 13/13: **Positionen ohne Lücke** bei übersprungenen leeren Zeilen (1,2 statt 1,3), Escaping neutralisiert `</script>` im Namen, Delete-Guard, kein XSS → 0 Bugs; aria-live am Status ergänzt; bot 0/0; html-validate exit 0. Viersprachig — build/row/renumber+Escaping byte-identisch; nur SEED-Namen/aria/Status/Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 43 (Site: 898 Familien / 3036 Seiten).
- **Tool #42 (22.06.) via 2tes-Gehirn:** CSS Text-Shadow-Generator (Regler X/Y/Unschärfe/Deckkraft, Color↔HEX, freier Beispieltext, Live-Vorschau, Copy text-shadow). Spiegelt Box-Shadow-Tool; neue Fläche Beispieltext via textContent → XSS-sicher (kein innerHTML). Review: Output-Order/Alpha/HEX-Sync, a11y → 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — clampHex/hexToRgb/val/render byte-identisch; nur Beispiel-Fallback + 2 Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 42 (Site: 897 Familien / 3032 Seiten).
- **Tool #41 (22.06.) via 2tes-Gehirn:** CSS Grid-Generator (Slider Spalten 1–12/Zeilen 1–8/Spalten-+Zeilen-Abstand 0–40, Live-Raster mit nummerierten Zellen, Container-CSS Copy). Review: Preview==Output exakt, iv()-Clamp, kein XSS (Zellen nur Integer-Counter), a11y → 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — render/iv byte-identisch; nur 2 Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 41 (Site: 896 Familien / 3028 Seiten).
- **Tool #40 (22.06.) via 2tes-Gehirn:** Platzhalter-Bild-Generator (Dummy-Bilder: Breite×Höhe 1–4000 + Presets + optionaler Text + BG/Text-Farbe → Canvas in echter Pixelgröße + skalierte Vorschau, Download PNG via toBlob). Review fand 1 Bug: Font-Floor konnte 7px statt 8px werden bei fraktionalem Init → Math.round; dim-Clamp (kein 0/>4000-Canvas), Shrink-Loop terminiert (~1192 max), toBlob leak-frei, kein XSS (Text nur fillText) → sonst 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — dim/render/download+PRESETS byte-identisch; nur Warn-String + Datei-Prefix je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt **40** (Site: 895 Familien / 3024 Seiten).
- **Tool #39 (22.06.) via 2tes-Gehirn:** CSS Flexbox-Generator (Selects für flex-direction/-wrap/justify-content/align-items + gap-Slider + Box-Anzahl 1–12, Live-Vorschau, Container-CSS Copy). Review testete alle **1800 Kombinationen**: Preview==Output exakt, Count-Clamps, kein XSS (keine Freitexteingabe), a11y → 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — render/setItems byte-identisch; nur 2 Flash je Sprache. 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 39 (Site: 894 Familien / 3020 Seiten).
- **Tool #38 (22.06.) via 2tes-Gehirn:** Favicon-Generator (Buchstabe/Emoji → Canvas 256px + 32/16-Vorschau; Form Kreis/abgerundet/eckig, BG+Text-Farbe, Schriftgröße, Fett; Download PNG via toBlob). Review: Canvas-Skalierung auf 16/32/256, roundRect-Pfad gültig, toBlob leak-frei (revokeObjectURL), kein innerHTML/XSS (Text nur via fillText), a11y (canvas role=img) → 0 Bugs; bot 0/0; html-validate exit 0. Viersprachig — inline-Script MD5-identisch (keine übersetzbaren JS-Strings). 4 Hubs, Sitemap 4/4. Tool-Suite jetzt 38 (Site: 893 Familien / 3016 Seiten).
