...
**📌 2026-07-04 (🧵 CJ-DAUER-IMPORT — „100000 cj produkten, kleider voll + sonstiges, ohne pause"):**
- **NEU: `automation/cj_perpetual.mjs`** — endloser Treiber. Reihenfolge: **KLEIDER zuerst tief** (`cjdamen`=Lady Dresses/Blusen/Jeans/… , `cjherren`; MAXPAGE 25, PERCAT 40, CAP 80) → dann **18 weitere saubere Gruppen breit** (Taschen/Schmuck/Uhren/Home/Beauty-Tools/Haustier/Elektronik/Gadgets/Skincare/Makeup/Nagel/Küche/Storage/Sport/Pet/Gaming/Musik/3D; CAP 40). Ruft je Gruppe `cj_category_fill.mjs` als Kind (DE-Titel via Gemini, Fehltreffer-Ban, Fashion-Varianten Farbe+Grösse, 6 Kanäle, Collection-Tags, Ledger `dropship/cj_niche_done.txt`).
- **`cj_category_fill.mjs`**: neue ENV **MAXPAGE** + **PERCAT** fürs tiefe „voll"-Füllen einer Kategorie.
- **🔑 HARTE GRENZE (ehrlich): CJ-TAGESPUNKTE ~50k/Tag.** Heute bereits **96'510 verbraucht → 0 frei** (code `16900500`). **100k Produkte gehen NICHT auf einmal** — CJ dosiert per Punkte. Der Treiber **erkennt 16900500 und WARTET auf den Tages-Reset** (30-Min-Schleife), grindet dann automatisch die vollen Punkte weiter. = „ohne pause" im Sinne von *läuft dauerhaft, resümiert selbst*. Realistischer Durchsatz grob paar-hundert–~1000 Produkte/Tag → 100k ist ein **Mehr-Tage/-Wochen-Grind**, nicht sofort. Nicht als „Defekt" missverstehen, wenn tagsüber wenig passiert = Punkte leer.
- **⚠️ Nur EINEN CJ-Treiber laufen lassen** (geteilter Punkte-Pool). Alten `/tmp/cj_loop.sh`-Supervisor + Streu-`cj_category_fill` gekillt; nur `cj_perpetual.mjs` bleibt. Start (Container-Neustart): `set -a; source /tmp/lux_env.sh; set +a; export CJ_TOKEN=$(cat /tmp/cj_token.txt|tr -d '[:space:]'); nohup /opt/node22/bin/node automation/cj_perpetual.mjs > /tmp/cj_perpetual.log 2>&1 &` (Token auch aus `/tmp/cj_token.json` autoladbar).
- **Container ephemer:** Hintergrundprozesse sterben beim Reclaim → jede neue Session den Treiber neu starten (Skript ist committet, Ledger idempotent = keine Dubletten). Durabler Weg wäre GitHub-Actions-Autopilot (Account-seitig noch deaktiviert, Owner-Klick).

**📌 2026-07-05 (🚬 KIFFER/RAUCHER-KATEGORIE endlich gefüllt — 46 echte, online-only):**
- **Wo waren die Kiffer-Sachen? NIRGENDS** — `cj_raucher` lief nie (Punkte leer + Container-Reset). Collection `raucherzubehoer` existierte leer.
...
**📌 2026-07-05 (🧹 GROSS-CLEANUP: Varianten-Merge + 20-Agenten-Kategorie-QA — „mach alles sauber, 20 agent"):**
- **DUBLETTEN → 1 Produkt mit Auswahl:** `automation/merge_variants.mjs` — führt Farb-/Grössen-Geschwister (separat importiert) zu EINEM Produkt mit Auswahl zusammen. **29 Gruppen** konsolidiert (Rucksacktrolley Safta 5 Farben, Casio-Damenuhr, Hama-Kopfhörer, Hunter-Halsbänder, MK-Taschen, Wanduhr Versa/Bohrer=Grössen …). **Fulfillment-sicher** (SKU je Variante erhalten), **reversibel** (Originale archiviert + URL-Redirect alt→neu), **Preis-Kohärenz-Schutz** (verschiedene Modelle nicht fälschlich gemergt — Lehre: „Kopfhörer Hama" 29.90 vs 112.90 hätte 2 Modelle vermischt), Umlaut-sichere Grössen-Regex (JS-`\b` frisst sonst „L" in „Lässiger"), Variante→Bild-Mapping. Test→Verify→Revert→Fix→Rollout-Disziplin bewährt.
- **20-AGENTEN-QA (je 1 Kategorie, parallel):** ~**219 falsch getaggte Produkte** korrigiert, **alle Collections auf sortOrder BEST_SELLING**. Highlights:
  - **Fitness: 11** (User-Kinderscooter + 6× Kinder-Go-Kart + Erotik-Ring + Auto-Licht + Lampe) — Root-Cause: eine CJ-Charge (IDs ~15446272xxx) klebte pauschal `fitness,outdoor,sport` auf alles.
  - **Damen: 103** (63 Herren, 38 Kinder/Baby, 2 Bettwäsche fälschlich `damen`).
  - **Herren: 13** Damen-Artikel · **Uhren: 5** (Swatch-Ringe/Schuhregale) · **Schmuck: 5** umgetaggt + 5 aus manueller Collection entfernt · **Sport: 2** (Auto-Scheinwerfer) · **Camping: 4** (PC-Kühler, BigBuy-Fehlübersetzung „Tragbare Kühlbox") · **Beauty: 2** (USB-Sticks) · **Haustier: 2**.
  - **Sauber (0 Fehler):** Auto(143), Garten(220), Werkzeug(731), Wohnen(1981), Sonnenbrillen(202), Küche(14).
- **🔴 STRUKTURELLER BEFUND (mehrere Agenten, offen — braucht ruleSet/Bridge-Tag, heikel blind):** Etliche Smart-Collections matchen per **TITLE CONTAINS** → Fehltreffer, die per Tag NICHT entfernbar sind:
  - **Taschen** (`sub-taschen`: TITLE Tasche/Rucksack) → 28 Fehltreffer: 18 Taschen**lampe**, 7 Taschen**rechner**, 3 Taschen**messer**.
  - **Baby-Kids** (TITLE Baby/Kinder/Spielzeug) → ~155: **BaB**yliss, Hunde**spielzeug**, Klein**kinder**-Uhr.
  - **Haustier** (TITLE Napf) → Saug**napf**-Halter etc. · **Sneaker** (TITLE sneaker) → Sneaker-Puzzle/-Socken.
  - **Fix-Weg:** Shopify kann „(A OR B) AND NOT C" nicht direkt → Bridge-Tag-Trick (positive Treffer taggen, dann TAG-Collection mit TITLE NOT_CONTAINS-Guards, appliedDisjunctively=false). ⚠️ collectionUpdate mit ruleSet WIPED seo → danach SEO neu setzen. Bewusst NICHT blind gemacht (kein Live-Screenshot-Check möglich).
  - **Nachtrag (letzte 2 Agenten):** **Gaming: 17** (Lotto-Maschine, Konsolen-Tische=Möbel, Hydraulik-Ventile, RAID-Controller — via „controller/joystick/Konsole"-Keywords). **Elektronik: 55** (Nike-Schuhe/Kleidung/Kostüme/Haushalt/Haustier — csv-Import-Refurbished-Charge taggte alles blind `elektronik`). **GESAMT ~219 Retags + 5 aus manueller Schmuck-Collection + 29 Merges + 20 Collections auf BEST_SELLING sortiert.**
  - **Wiederkehrender Root-Cause:** Import-Chargen (CJ + csv/bigbuy-refurbished) kleben pauschal breite Tags (fitness/outdoor/sport, elektronik/tech, hund/katze/pet) auf ganze Batches → Fehltreffer. Beim Import strenger taggen; periodisch QA-Sweep pro Kategorie.

**📌 2026-07-05 (🏎️ MODELLAUTOS-NISCHE — User zeigte DCM-Works-Diorama-Reel):**
- **Diorama/Display-Bases selbst (DCM Works 7-Eleven DriftPad, 1:64) = NICHT dropship-fähig** — CJ hat 0 Dioramen/Modellauto-Vitrinen/1:64-Display (Suche liefert nur Müll). Bräuchte spezialisierten Diecast-Grosshändler (User-Lieferanten-Anbindung).
