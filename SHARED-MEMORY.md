...
> **⭐ STEHENDE REGEL (User 2026-06-16): „Bestseller immer aktuell halten".** Collection **„🔥 Hero-Favoriten"** (`gid://…/Collection/687789113729`, handle `bestseller`, MANUAL) jede Session frisch halten. Mechanik: **`automation/update_bestsellers.mjs`** rankt nach **echten Verkäufen (60 T)**; bei **0 Verkäufen** (aktueller Stand) Fallback = **Review-Sieger** (Metafelder `reviews.rating`/`rating_count`, ≥4,5★/≥3) + **beworbene Premium-Produkte** aus `good_products.csv`, aufgefüllt auf 12, Review-Sieger zuerst. **Workflow `bestseller-refresh.yml`** (täglich 06:00 UTC) fährt das unbeaufsichtigt, SOBALD Shopify-Secrets gesetzt + App installiert sind (sonst no-op). **Solange kein Cron/App:** in jeder Dropship-Session via Shopify-MCP nachziehen (collectionAddProducts/RemoveProducts/ReorderProducts). Aktueller Stand (2026-06-16): 11 Produkte, 4 echte Review-Sieger zuerst (Slim Wallet 5★/15, Herrenuhr 5★/15, Jade-Roller 5★/7, Robo-Diffuser 4,8★/4), Dublette (2. Jade-Roller) raus, +Moissanite «Aurora»/Tasche «Milano»/Kopfhörer «Aria».
