...
- **Interne Verlinkung (53 Artikel ohne `/collections/`-Link):** themen-passende „🛍️ Passend dazu bei LuxeStyle"-Box (2 Collection-Links je Artikel, deterministisch aus Titel-Keywords, nur verifizierte Handles). **~23 live angehängt; ~30 OFFEN** wegen **Anthropic-529-Overload** (Subagents starteten nicht). **Idempotent fortsetzbar:** Mapping liegt in **`dropship/article_shop_links.json`** (53 Einträge {id,title,links}); Runner = Subagent, der je Artikel `article{body}` liest, bei vorhandenem `/collections/` SKIPpt, sonst Box anhängt (`articleUpdate` mit `$body` als Variable). Bei Overload einfach erneut laufen lassen — fertige werden übersprungen.
