...
#   (2) Auto-Fulfillment + Auto-Billing an, (3) erste Bestellung #1005 BLANK XL/Rot als manuelle Printful-Order an
#   Alain Schutz, Rue du Pavement 24, 1530 Payerne, +41 79 453 79 86 (bezahlen = vom User freigegeben, ~CHF 15-25).
...
**🔴 UPDATE 2026-06-25 (ehrlich, nach mehreren Läufen):** Der **Browser-Bot kann den Wizard NICHT öffnen** — er hängt auf der Kampagnen-Liste, die `ai-objective`/`ai-bid`-Screenshots sind FEHLBENANNT (zeigen die Liste), **es wurde KEINE Kampagne erstellt** (nur eine alte „Sales2026-06-06", 0 CHF). Auch `ab.act()` + Direkt-URL öffnen den Wizard nicht. → **Browser-Weg aufgegeben.** VERLÄSSLICH: **(A)** Marketing-API `tiktok-campaign-api.mjs` (braucht `TT_MKT_TOKEN` in luxe-secrets — bitte besorgen/eintragen, dann läuft's deterministisch, hart budget-gedeckelt) **ODER (B)** manueller 5-Min-Launch mit GENAU dieser Config: Objective **Traffic** · Standort **Schweiz** · Alter **18-34** · Sprache **de+fr** · Placement **nur TikTok** · Pixel **D8EKVR3C77U6KT5BTBD0** · **Lifetime-Budget 70 CHF** (≤350) · Creative = sauberes Wasserfest-/See-Test-Reel · CTA **Shop Now** · Landing (Auto-Apply -10%) `https://luxestyle.ch/discount/WELCOME10?redirect=/collections/wasserfester-schmuck` · Ad-Text „Wasserfester Schmuck, der bleibt – aluffrei oder Geld zrugg". **[ANDERE SESSION]** kann (A) oder (B) übernehmen.
**🔴 TikTok-DUBLETTEN löschen:** Browser-Delete klappt NICHT (`reports/tiktok-delete.json` deleted:0, TikTok hat keine Delete-API) → **User löscht die 2-3 Dubletten (weisser Sneaker, beiger Polo) in der TikTok-App (3 Taps)**. Neue Dubletten sind durch Dedup verhindert.
...
0. 🎯 **PIXEL FEUERT NICHT = HÖCHSTE PRIO „egal wie" (User 2026-06-22):** TikTok-Pixel `D8EKVR3C77U6KT5BTBD0` ist eingebaut, aber **consent-gesperrt** (Cookie-Banner) → daten-leer. **FIX:** Admin → Einstellungen → **Kundendatenschutz/Cookie-Banner** → da Shop **CH-only (nicht EU/GDPR)** Tracking **ohne Opt-in-Pflicht** zulassen / Consent-Default = akzeptiert → Pixel feuert. (Consent-Mode = Theme/Settings, nicht per Admin-API flippbar.) **WICHTIG: entsperrt ALLE 4 Pixel-Kanäle gleichzeitig** (TikTok + Meta/Facebook&Instagram + Google&YouTube + Pinterest sind alle installiert & consent-gated). Pro Kanal danach „Datenfreigabe = max/aktiv" prüfen (TikTok=done; Meta/Google/Pinterest = Login nötig → PC/User).
