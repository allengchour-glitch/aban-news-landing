...
- **Offen (Option a, optional):** ~1.282 verbleibende Produkte ohne Barcode sind **CJ/POD** = haben gar keine EAN → korrekt wäre `mm-google-shopping.custom_product=true` (Boolean-Metafield-Def existiert) → „kein Identifier" → Warnung weg. Nicht blockierend (gelb). **5 FAILED-Bilder** = einzige echte Disapprovals → PC-Claude.
