...
> **🔑 ZUGRIFF (User 2026-06-16: „du hast den Token, der hat alle Zugriffe — merk dir das"):** **In-Session habe ich via Shopify-MCP VOLLEN Schreibzugriff** (bewiesen: hunderte productUpdate/collectionUpdate/publishablePublish/articleUpdate live). **→ Regel: nicht auf einen „Unlock" warten, sondern direkt per MCP machen.** ⚠️ Getrennt davon: der **Standalone-/Cron-Token-Pfad** (`/tmp/shop_creds.json`, client_id + `shpss_`-Secret) liefert weiterhin **`app_not_installed` (HTTP 400, am 2026-06-16 erneut verifiziert)** — `shpss_` ist nur der Client-Secret, kein Admin-Token. Für **unbeaufsichtigte** Läufe (GitHub-Cron / Cloudflare-Worker) braucht es daher entweder die **Custom-App 1× im Shop installiert** (dann greift Client-Credentials) ODER einen echten **`shpat_`/`shpca_`-Token** als Secret. Die Autonomie-Tools sind dafür gebaut + no-op-sicher; in-session werden sie über MCP gefahren.

**2026-06-16 (Katalog-Session) — 🐞 PAGE-BUG: kaputte „Inspiration"-Bilder auf `/pages/selbst-gestalten` (User-Screenshot „kein Bild, nur Sticker"):** Die 6 Inspiration-`<img>` zeigten auf die **Fremd-Domain `abannews.com/social/looks/ex-*.png`** (anderes Projekt) → luden nicht = Broken-Image-Icons. **Fix:** durch funktionierende `cdn.shopify.com`-Produktbilder ersetzt (Page-id 698444710273, `pageUpdate` — body-Arg ist `String!`, nicht `HTML!`). 0 abannews-URLs mehr, 0 Fehler. **→ Lehre/Hunt:** Fremd-Domain-Verweise = „ähnliche Fehler" → shopweite Suche nach `abannews.com` in Pages/Articles/Collections/Produkten gestartet.
