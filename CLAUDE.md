...
- `video-prototypes/` — **Säule 2 des Video-Stacks** (aufwändiger): faceless **YouTube-Kanal „ABAN Files"** mit Pexels-Stock + optional HeyGen-Avatar + ElevenLabs (`scripts/eleven_pipeline.py`) + Whisper-Untertitel; `aban-files/aban_render.py`/`aban_publish.py` + Workflow `aban-youtube.yml` rendern **in der CI** und laden als Short hoch (**Zeitplan deaktiviert**, nur `workflow_dispatch`). Braucht Secrets `XI`/`PEXELS`/`YT_*`. Renders (`output/`, `*.mp4`) **git-ignored** — die früheren 275 MB wurden aus Git entfernt; `youtube.yml` (lud vorab-gerenderte Clips) ist **veraltet/deaktiviert**. **Beide Säulen + Keys: `docs/VIDEO-STACK.md`** (Quelle der Wahrheit). Ehrlich: Auto-Publish lohnt nur mit regelmäßigem, gutem Content.
