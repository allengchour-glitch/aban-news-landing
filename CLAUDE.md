...
- `luxestyle-3d/` — **3D-Druck-Pipeline für Abans EIGENEN Shop `luxestyle.ch`** (Shopify Basic, CHF, Schweiz) — eigener Geschäftszweig, **getrennt von aban news** (eigene Social-Kanäle nutzen, NICHT die aban-Kanäle). Macht aus Text/Bild **Vollkörper-3D-Minifiguren** (personalisierte Geschenke: Tier-Anhänger, Namensschild, Cake-Topper). **FESTE KUNDENREGEL: keine flachen Teile — nur rundherum modellierte 3D-Figuren.** Pipeline: `meshy_text.py`/`meshy_image.py` (Meshy AI text/image→3D, braucht `MESHY_API_KEY`) → `polish.py` (Remesh/Dezimieren/Basis/Render) → optional `hole.py` (Ring-Loch), `colorize.py`/`assemble.py` (Mehrfarb-AMS). Druck: FDM Bambu X1C/AMS, manifold, flache Unterseite, Figur ~40–50 mm. Freigegeben: Loaf-Katze „Mimi" + Loaf-Hund (Prompts im README). `posts.json` = fertige LuxeStyle-Posts für `social/post.py` (eigene Kanäle, erst senden wenn Produkte live). **Erst Sample, dann Shopify; live schalten nur der Inhaber.** Volle Doku: `luxestyle-3d/README.md`. (Zuerst auf Branch `claude/neujes-dropshipping-project-oO7SW` gebaut — Memory hier verankert, damit jede Session es kennt.)
...
- `video-pipeline/` — **Faceless-Kurzvideo-Fabrik**: macht aus den Hype-Watch-Faktenchecks (`data/hype-watch.json`) fertige 9:16-Clip-Pakete (4 markengetreue Slide-PNGs via Pillow + Voiceover-Skript + `untertitel.srt` + Social-`post.txt`). `generate_clips.py` (stdlib + Pillow). Optionale Vertonung via `ELEVENLABS_API_KEY` (`--voice`, sonst übersprungen); HeyGen-Rendering bewusst nicht automatisiert (Kosten/Clip). **Ehrlich:** Geld kommt indirekt über den Funnel (kein YouTube-Reichtums-Versprechen); nur belegte Faktenchecks werden vertont. Verteilung über `social/post.py`. Output `ausgabe/` git-ignored. Selbst-wachsend: jeder neue Hype-Watch-Fall wird zum Clip. Build `cd video-pipeline && python generate_clips.py`. Dazu `generate_promo.py` — produktionsfertiges Paket für den **aban-news-Werbespot** (Varianten A 40 s + B 15 s aus `docs/WERBEVIDEO-SKRIPT.md`): Storyboard-Frames (9:16) + `voiceover.txt` (→ ElevenLabs) + `untertitel.srt` + `regie.txt` (→ HeyGen). `--voice` für ElevenLabs-VO. Build `python generate_promo.py`. **Reels fertig rendern (autonom):** `render_all_reels.py` baut Slides + rendert daraus via `mediakit.reel` direkt fertige 9:16-mp4s (Slides+Untertitel+Ambient); der Workflow `video-pipeline-build.yml` läuft bei Änderung an `video-pipeline/`, `mediakit/` oder `data/hype-watch.json`, lädt die fertigen `reel.mp4` als Artefakt `aban-reels-fertig` hoch (kein CapCut mehr). Mit Secret `XI` automatisch Stufe A (Stimme+Karaoke), sonst Stufe B/C.
- `mediakit/` — **gemeinsames Bild- & Video-Bearbeitungs-Toolkit** (Pillow + optional ffmpeg via `imageio-ffmpeg`), wiederverwendbar für beide Video-Projekte. CLI `python3 -m mediakit …`. **3 Fähigkeiten:** (1) Allzweck-Edit — `image crop|resize|pad|thumb|overlay|caption` (nur Pillow), `video trim|concat|vertical|captions|music|fade|thumb`; (2) **Reels fertig rendern** (schließt die CapCut-Lücke): `reel <case-id>` macht aus `video-pipeline/ausgabe/<case>/` (Slides+Skript+SRT) ein fertiges 9:16-mp4 mit eingebrannten Untertiteln + ruhigem Ambient-Pad. Timing 3-stufig: **A** `--voice`+`XI`-Key → ElevenLabs-Wort-Timings (audiosynchron + Karaoke), **B** SRT-Cues, **C** feste Dauern. Output `ausgabe/<case>/reel.mp4` (git-ignored); (3) **Kreativ-KI** via Adobe-Firefly-MCP — **agenten-aufgerufen** (kein Skript), `firefly prepare/ingest` staged nur Dateien (Doku `mediakit/FIREFLY.md`). **`mediakit/brand.py`** ist jetzt die Single Source of Truth fürs Markenkit (Farben/Fonts/`slide`) — `video-pipeline/generate_clips.py` + `generate_promo.py` importieren von dort (Verhalten identisch). ffmpeg/ASS/Audio/TTS-Muster aus `aban-files/aban_stock.py` **portiert** (inkl. ASS-`Name`-Feld-Fix); die live laufende aban-files-Pipeline bleibt **unangetastet**. Bild-Befehle laufen ohne ffmpeg; Video braucht `pip install imageio-ffmpeg`. Doku `mediakit/README.md`.
