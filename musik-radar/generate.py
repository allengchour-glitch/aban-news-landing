#!/usr/bin/env python3
"""Musik-Radar — static site generator (German, DACH-Fokus).

Reads the curated KI-music-tool database (data/anbieter.json) and an
...
du-Form — KI-Musik ist hype-lastig, hier wird ehrlich verglichen.
...
SITE_NAME = "Musik-Radar"
BASE_URL = "https://musik.abannews.com"
LANG = "de"
...
UI = {
    "tagline": "KI-Musik-Tools (Song-Generierung, Royalty-Free, Stems), ehrlich verglichen — für den DACH-Raum",
    "all_tools": "← Alle Tools",
    "categories": "Kategorien",
    "focuses": "Schwerpunkt",
    "home_heading": "{n} KI-Musik-Tools im Überblick",
    "provider": "Anbieter",
    "type": "Typ",
    "focus": "Schwerpunkt",
    "language": "Sprache",
    ...
    "best_in": "Beste KI-Musik-Tools: {cat}",
    "focus_page": "KI-Musik-Tools: {foc}",
    "privacy": "Datenschutz",
    "imprint": "Impressum",
    ...
    "honest": ("Ehrlich vorab: KI-Musik klingt oft brauchbar, aber generisch — als Hintergrund top, als Hit selten. "
               "Der heiklere Punkt ist die Rechtslage: Wem gehört der Song, was darfst du kommerziell nutzen? "
               "Das hängt am Tarif und ist je Anbieter verschieden; einige stecken in Urheberrechts-Streits. "
               "Prüfe vor kommerzieller Nutzung immer die Lizenz. Dieser Radar vergleicht Werkzeuge — "
               "er verspricht weder den Hit noch wasserdichte Rechte."),
    "meta_home": ("{n} KI-Musik-Tools im Vergleich: "
                  "Song-Generierung mit Gesang, Royalty-Free für Content, Stems & Mastering, "
                  "Open Source — ehrlich, mit Blick auf Lizenz/kommerzielle Nutzung."),
    "meta_tool": "{name} ({anbieter}): Typ, Schwerpunkt, Integrationen, EU-Hosting und offizieller Link.",
    "meta_cat": "Die besten KI-Musik-Tools im Bereich {cat} — ehrlich verglichen für DACH.",
    "meta_foc": "KI-Musik-Tools mit Schwerpunkt {foc} — Funktionen und offizielle Links im Vergleich.",
}
...
FAVICON_SVG = (
    ...
    '<path d="M40 16v22a8 7 0 1 1-4-6V22l-14 3v17a8 7 0 1 1-4-6V20z" fill="#fff"/></svg>'
)
...
def faq_section(tool):
    ...
    # Deutsche Oberfläche / Songtexte (DACH-relevant)
    qa.append((f"Versteht {name} Deutsch und hat es eine deutsche Oberfläche?",
               {"ja": f"Ja, {name} bietet laut Anbieter eine deutschsprachige Oberfläche bzw. deutschsprachige Songtexte.",
                "nein": f"Nein, {name} ist primär englischsprachig — deutsche Songtexte vor dem Einsatz testweise prüfen.",
    ...
    qa.append((f"Hostet {name} in der EU (DSGVO-relevant)?",
               {"ja": "Ja, laut Anbieter gibt es EU-Hosting bzw. EU-Datenverarbeitung — das erleichtert die DSGVO-Konformität, gerade beim Hochladen eigener Audio-/Textvorlagen.",
                "nein": "Nein, die Datenverarbeitung erfolgt überwiegend außerhalb der EU — Auftragsverarbeitung (AVV) und Drittlandtransfer (z. B. Standardvertragsklauseln) selbst prüfen, bevor du fremdes Audio/fremde Stimmen hochlädst.",
...
def home_page(tools, categories, focuses, aff):
    ...
    body = (f'{site_schema()}{itemlist(tools)}'
            ...
            f'„{e(UI["eu_stock"])}“ ist der wichtigste DACH-Filter: DSGVO-näher, EU-Datenverarbeitung, weniger Drittland-Aufwand beim Upload eigener Audios.</p>\n'
...
LEGAL_DOMAIN = "musik.abannews.com"
...
beim Einsatz von KI-Musik-Tools (z. B. Urheber-/Nutzungsrechte am erzeugten Song,
Auftragsverarbeitung nach Art. 28 DSGVO, ggf. Drittlandtransfer beim Upload) liegen bei dir.</p>
...
<h2>6. Hinweis zu Uploads & Lizenz bei den verglichenen Tools</h2>
<p>Die hier verglichenen KI-Musik-Tools laufen bei den jeweiligen Anbietern, nicht auf dieser
Website. Lädst du dort eigene Audios/Texte hoch oder klonst Stimmen, brauchst du die Rechte
daran; wem der erzeugte Song gehört und was du kommerziell nutzen darfst, regelt der Anbieter-Tarif. Hosting-Standort, Auftragsverarbeitung (AVV) und
...
def notfound_page():
    body = (f'<h1>🎵 404 — Seite nicht gefunden</h1>\n'
            f'<p class="meta">Diese Seite gibt es nicht. Zurück zur Übersicht aller KI-Musik-Tools.</p>\n'
...
SEARCH_JS = """// Musik-Radar — client-side filter (no deps, no tracking)
(function () {
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Musik-Radar generator")
