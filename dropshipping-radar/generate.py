#!/usr/bin/env python3
"""Dropshipping-Radar — static site generator (German, DACH-Fokus).

Reads the curated dropshipping-provider database (data/anbieter.json) and an
...
site:
  ...
  - one page per provider (SoftwareApplication schema + FAQ + breadcrumb)
  - one page per category (tool type), one page per market focus
  - sitemap.xml, robots.txt, RSS feed, legal pages, 404

Structurally modelled on kurse-radar/generate.py: same brand tokens, the same
...
Dropshipping ist ein hype-lastiges Feld, hier wird ehrlich verglichen.
...
SITE_NAME = "Dropshipping-Radar"
BASE_URL = "https://dropshipping.abannews.com"
LANG = "de"
...
UI = {
    "tagline": "Dropshipping-Tools & -Lieferanten, ehrlich verglichen — für den DACH-Raum",
    "all_tools": "← Alle Anbieter",
    "categories": "Kategorien",
    "focuses": "Markt-Fokus",
    "home_heading": "{n} Dropshipping-Anbieter im Überblick",
    "provider": "Betreiber",
    "type": "Typ",
    "focus": "Markt-Fokus",
    "language": "Sprache",
    "topics": "Schwerpunkte",
    "format": "Art",
    "integration": "Shop-Anbindung",
    "de_ui": "Deutsche Oberfläche",
    "eu_stock": "EU-/DACH-Lager",
    "price": "Preis",
    "score": "Wertung",
    "editor_note": "Einordnung",
    "to_provider": "Zum Anbieter",
    "best_in": "Beste Dropshipping-Anbieter: {cat}",
    "focus_page": "Dropshipping-Anbieter: {foc}",
    "privacy": "Datenschutz",
    "imprint": "Impressum",
    ...
    "search_ph": "Anbieter, Lieferant oder Tool suchen…",
    ...
    "honest": ("Ehrlich vorab: Dropshipping ist Arbeit, kein Selbstläufer. Margen, "
               "Versandzeiten, Retouren und Rechtspflichten (Impressum, Widerruf, "
...
FAVICON_SVG = (
    ...
    '<path d="M32 12 12 22v6l20 10 20-10v-6z" fill="none" stroke="#fff" stroke-width="3" '
    'stroke-linejoin="round" stroke-linecap="round"/>'
    '<path d="M12 28v14l20 10 20-10V28" fill="none" stroke="#fff" stroke-width="3" '
    'stroke-linejoin="round" stroke-linecap="round"/>'
    '<path d="M32 38v14" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"/></svg>'
)
...
def flag(label, value):
    """Farb-Badge für ja/nein/unbekannt (EU-Lager, deutsche Oberfläche)."""
    if value is True:
...
def faq_section(tool):
    ...
    qa.append((f"Hat {name} eine deutsche Oberfläche?",
               {"ja": f"Ja, {name} bietet laut Anbieter eine deutschsprachige Oberfläche.",
                "nein": f"Nein, {name} ist primär englischsprachig.",
                "—": "Bitte beim Anbieter prüfen — wir raten hier nichts."}[bool_str(tool.get("deutsche_oberflaeche"))]))
    # EU-/DACH-Lager (der zentrale DACH-Faktor)
    qa.append((f"Liefert {name} aus einem EU-/DACH-Lager?",
               {"ja": "Ja, es gibt EU-Lager bzw. EU-nahe Produktion — das verkürzt die Lieferzeit nach DACH.",
                "nein": "Nein, der Versand erfolgt überwiegend nicht aus der EU — längere Lieferzeiten und ggf. Zoll/Einfuhr einplanen.",
                "—": "Teilweise/produktabhängig — vor dem Start je Produkt beim Anbieter prüfen."}[bool_str(tool.get("eu_lager"))]))
    # Shop-Anbindung
    integ = ", ".join(tool.get("integration", [])) or "—"
    qa.append((f"Mit welchen Shop-Systemen funktioniert {name}?",
               f"Laut Anbieter u. a.: {integ}."))
    # Eignung
    qa.append((f"Für wen eignet sich {name}?",
               f"Typ: {', '.join(tool.get('kategorie', [])) or '—'}. "
               f"Markt-Fokus: {tool.get('fokus', '—')}."))
    details = "".join(
...
def tool_page(tool, aff, related):
    ...
    if related:
        links = " · ".join(
            ...
            for r in related)
        rel_html = f'<p class="subnav"><strong>Ähnliche Anbieter:</strong> {links}</p>'
...
def home_page(tools, categories, focuses, aff):
    ...
    body = (f'{site_schema()}{itemlist(tools)}'
            ...
            f'„{e(UI["eu_stock"])}“ ist der wichtigste DACH-Filter: kürzere Lieferzeit, EU-Rechnung, weniger Zoll.</p>\n'
...
LEGAL_DOMAIN = "dropshipping.abannews.com"
...
<p>Unser Angebot enthält Links zu externen Websites Dritter (Tool-Anbieter und Lieferanten),
auf deren Inhalte wir keinen Einfluss haben. Für diese fremden Inhalte ist stets der jeweilige
Anbieter verantwortlich. Preise, Konditionen, Lager-Standorte und Verfügbarkeit können sich
beim Anbieter jederzeit ändern — maßgeblich ist die Anbieterseite.</p>
<h2>Kein Rechts-/Steuerberatung</h2>
<p>Dieser Radar vergleicht Werkzeuge und ersetzt keine Rechts- oder Steuerberatung. Pflichten
beim Dropshipping (z. B. Impressum, Widerrufsrecht, Produktsicherheit, Umsatzsteuer/OSS,
Verpackungs- und Elektrogesetz) liegen beim Händler.</p>
...
<p>Auf Anbieterseiten verlinken wir auf Tools und Lieferanten. Erst wenn du einen solchen Link
...
def notfound_page():
    body = (f'<h1>📦 404 — Seite nicht gefunden</h1>\n'
            f'<p class="meta">Diese Seite gibt es nicht. Zurück zur Übersicht aller Dropshipping-Anbieter.</p>\n'
...
SEARCH_JS = """// Dropshipping-Radar — client-side filter (no deps, no tracking)
(function () {
...
def build(data_path: Path, aff_path: Path, out: Path) -> int:
    ...
    # Sortierung: EU-Lager zuerst (DACH-Relevanz), dann geprüfte Wertung, dann alphabetisch.
    ...
    # Ähnliche Anbieter: bis zu 4 mit gemeinsamer Kategorie.
    related_map = {}
    for t in tools:
        seen, rel = {t["id"]}, []
        for cat in t.get("kategorie", []):
    ...
    print(f"Built {pages} HTML pages from {len(tools)} providers "
          f"({len(used_cats)} Kategorien, {len(used_focuses)} Fokus-Seiten) + sitemap + RSS → {out}/")
    return pages


def main():
    here = Path(__file__).resolve().parent
    ap = argparse.ArgumentParser(description="Dropshipping-Radar generator")
