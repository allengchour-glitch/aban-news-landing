...
let seq=0;
function curlGet(u){return new Promise(res=>{const f='/tmp/_bs'+(seq++);
...
const out=await p.evaluate(()=>{
 ...
 const body=document.querySelector('.shopify-policy__body')||document.querySelector('main');
 const hits=[];
 for(const ch of [...body.children]){
  const old=ch.style.display; ch.style.display='none';
  const w=document.documentElement.scrollWidth;
  ch.style.display=old;
  if(w<base-2) hits.push({tag:ch.tagName, cls:(typeof ch.className==='string'?ch.className:''), newW:w, txt:(ch.textContent||'').trim().replace(/\s+/g,' ').slice(0,90), html:ch.innerHTML.slice(0,200)});
 }
 // Zusatz: alle Nachfahren mit scrollWidth > 390
 const over=[];
 for(const el of body.querySelectorAll('*')){ if(el.scrollWidth>392 && el.children.length===0) over.push({tag:el.TagName||el.tagName, sw:el.scrollWidth, cw:el.clientWidth, txt:(el.textContent||'').trim().slice(0,80)}); }
 return {base, hits, over: over.slice(0,10)};
});
