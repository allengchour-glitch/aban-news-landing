#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""sticker_druckdatei.py — hebt die Druckdateien der POD-Sticker auf die Shopify-CDN.
...
LEDGER = "dropship/_sticker_druckdatei.txt"
...
def main():
    ...
    while True:
        ...
        for p in pg["nodes"]:
            ...
            if p["id"] in erledigt:
                continue
            datei = os.path.join(DESIGNS, os.path.basename(v))
    ...
    for t, f in fehlt[:10]:
        print(f"   ⚠️ {t[:52]:52} — {f} fehlt in {DESIGNS}/")
    if DRY:
