...
def _gencfg(max_tokens: int, temperature: float, thinking_budget, response_json=False):
...
def _vertex(prompt: str, model: str, max_tokens: int, temperature: float,
            thinking_budget=None, response_json=False) -> str | None:
    ...
    body = {"contents": [{"role": "user", "parts": [{"text": prompt}]}],
            "generationConfig": _gencfg(max_tokens, temperature, thinking_budget, response_json)}
...
def generate(prompt: str, max_tokens: int = 4000, temperature: float = 0.4,
    ...
    response_json=True erzwingt natives JSON (responseMimeType) — robuster zu parsen."""
    for model in _candidates():
        out = _vertex(prompt, model, max_tokens, temperature, thinking_budget, response_json)
        if out:
            return out
