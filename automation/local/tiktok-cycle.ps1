...
#   3) NAeCHSTES Top-Reel auf TikTok posten (idempotent, Hero zuerst)   4) TikTok ANALYSIEREN + lernen
#   5) done-Ledger + Reports zurueck ins Repo committen.
# TikTok hat KEINE Post-API -> laeuft ueber den eingeloggten Brave (Port 9222). Das ist der saubere Weg
# fuer "mehrmals am Tag" ohne API: dieselbe Task einfach zu mehreren Uhrzeiten triggern.
#
...
#   (Mehr/weniger Uhrzeiten = mehr/weniger Posts/Tag. 2-3 ist fuer TikTok optimal; >4 riskiert Drosselung.)
#
...
# 1) Brave mit Debug-Port starten, falls noetig
...
# 2b) WORKER SELBST-DEPLOY (maximum auto): deployt den Cloudflare-Worker NUR wenn sich der Code geaendert hat.
powershell -ExecutionPolicy Bypass -File "$PSScriptRoot\worker-autodeploy.ps1" 2>$null

# 3) NAeCHSTES Top-Reel posten (Hero zuerst, idempotent via tiktok-upload-done.txt)
...
#    a) eingeloggte Analyse ueber den Bot (genauer, private Metriken) - b) oeffentliche yt-dlp als Fallback
...
# 4b) AUTO-AUFRAeUMEN (User 2026-06-19 "automatisch"): Verlierer loeschen - NUR 1/Tag (fruehester Lauf < 12 Uhr),
#     mit harten Sicherheits-Caps: max 3/Lauf, unter 60 Views, Top-5-Gewinner + tiktok-keep.txt geschuetzt, Ledger.
if ((Get-Date).Hour -lt 12) {
  node "automation/local/tiktok-bot.mjs" delete --losers --min-views 60 --max 3 --go 2>$null
}

# 5) Gelerntes + Ledger zurueck ins Repo (defensiv: erst rebase, dann push)
