...
try {
  ...
  foreach ($c in $cmds) {
    ...
    switch ($c) {
      ...
      "tt-delete-sneaker" { $env:PLATFORM="tiktok"; $env:MATCH="a white leather sneaker with a stripe and beige sole (weisser Sneaker Marco)"; & node "automation/local/tiktok-delete-dupes.mjs" --go --n 1; git add -f automation/local/tiktok-delete-shots/* reports/tiktok-delete.json 2>$null; git commit -m "auto(tt-delete-sneaker): 1 weisse-Sneaker-Dublette geloescht (behaelt 1)" 2>$null; git pull --rebase origin claude/luxestyle-product-CizQ6 2>$null; git push origin claude/luxestyle-product-CizQ6 2>$null }
