...
try {
  ...
  foreach ($c in $cmds) {
    ...
    switch ($c) {
      ...
      "smartplus-dry" { Ensure-Campaign; $env:CDP_HOST="127.0.0.1"; & node "automation/vps/smartplus-drive.mjs"; $env:CDP_HOST=$null; Write-Host "smartplus-dry fertig -> Stand in Shopify-Metafeld luxe.smartplus_status (kein Geld)" }
