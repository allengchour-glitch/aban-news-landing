...
try {
  ...
  foreach ($it in $r.commands) {
    ...
    switch ($c) {
      ...
      "yt-learn"     { Start-Process powershell -WindowStyle Hidden -ArgumentList '-ExecutionPolicy','Bypass','-Command',"`$env:YT_URL='https://www.youtube.com/watch?v=5gXwjXEG6P0'; node automation/yt-learn.mjs; git add reports/; git commit -m auto-ytlearn; git pull --rebase origin claude/luxestyle-product-CizQ6; git push origin claude/luxestyle-product-CizQ6" }
