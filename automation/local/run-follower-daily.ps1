# run-follower-daily.ps1 - LuxeStyle Marketing OHNE Befehl (fuer Windows-Taskplaner)
...
# lernen + Autopost-Queue neu bauen (auto.sh-Kette, Windows-tauglich) und ins Repo zurueckspeichern.
# IG/FB-POSTEN laeuft separat & ohne PC ueber den Cloudflare-Worker.
#
# EINMALIGE EINRICHTUNG (dann taeglich von selbst, kein Tippen):
...
#        $env:META_ACCESS_TOKEN = "EAA..."                 # fuer IG/FB-Kommentar-Auto-Antwort
#        $env:IG_USER_ID = "17841480560863361"           # @luxestyle.ch
#        $env:FB_PAGE_ID = "1049840534888592"            # LuxeStyle CH
#        $env:SHOPIFY_CLIENT_ID     = "..."                 # Custom-App (fuer Feed-Politur)
...
#          #  HeyGen trennt API-Credits  Dashboard-Credits -> API-Quota in HeyGen aufladen, sonst INSUFFICIENT_CREDIT
...
# 0) SELBST-BOOTSTRAP ("Bot wo klickt", User 2026-06-17): wenn dieser Task laeuft (z.B. alte
...
$loc = $PSScriptRoot
# (a) PC-Listener starten, falls noch keiner laeuft (1 node-Prozess mit pc-listener im Cmdline)
...
# 1) Brave mit Debug-Port starten, falls Port nicht lauscht (fuer Follower-Wachstum)
...
# 4) TikTok-Posten laeuft jetzt in der EIGENEN, mehrmals-taeglichen Task `tiktok-cycle.ps1`
...
# 5b) IG-Posts LOeSCHEN die nicht erlaubt sind (Botox/Serum/Rosehip/Gua-Sha) - IG-API kann published
#     NICHT loeschen -> Browser/CDP. Liest automation/local/ig-delete-queue.txt, idempotent. No-op wenn leer.
...
# 8b) KATALOG-FEED-POLITUR (autonom, User 2026-06-16 "ds soet scho i richtige kategorie si"):
#     setzt katalogweit die richtige Shopify-/Google-Produktkategorie + condition/gender/age.
#     Braucht SHOPIFY_CLIENT_ID/SECRET in luxe-secrets.ps1. Idempotent + resuemierbar -> MAX/Tag,
#     der naechste Lauf macht weiter, bis der ganze Katalog poliert ist. No-op ohne Creds.
...
if ($env:SHOPIFY_CLIENT_ID -and $env:SHOPIFY_CLIENT_SECRET) {
  ...
  # 8c) MODE-BESCHREIBIGE: Google-"Key details" (Farbe/Groesse/Material/Muster) ergaenzen
  #     (Merchant-Report "Update product descriptions"). Idempotent -> MAX/Tag, resuemierbar.
...
#     User 2026-06-17 "fuelle maximum, ma nuem de token igae". 10/Tag = stetig, ohne BigBuy-Quota zu sprengen.
...
#    Kette wie auto.sh, aber Windows-tauglich via node/python direkt (kein bash noetig). Alles no-op-safe:
...
#     no-op-safe ohne Keys. Multi-Provider-Router faellt automatisch zurueck, falls einer ausfaellt.
...
# 10) Gelerntes Gehirn + frische Queue zurueck ins Repo (sonst geht das Lernen verloren; der Worker
#     baeckt die Queue beim naechsten `wrangler deploy` ein). Defensiv: erst rebase-pullen, dann pushen.
...
git commit -m "auto(PC-Task): taegliche Analyse + Gehirn-Lernen + frische Queue" 2>$null
