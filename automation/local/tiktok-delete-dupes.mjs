...
const SHOT = path.join(ROOT, 'automation', 'local', 'tiktok-delete-shots');
fs.mkdirSync(SHOT, { recursive: true });
const GO = process.argv.includes('--go');
const N = (() => { const i = process.argv.indexOf('--n'); return i > -1 ? parseInt(process.argv[i + 1] || '3', 10) : 3; })();
const MATCH = process.env.MATCH || 'Lieblingsstuck';
...
(async () => {
  ...
  try {
    await p.goto(`https://www.tiktok.com/@${PROFILE}`, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await sleep(6000);
    ...
    for (let i = 0; i < N; i++) {
      try {
        if (ab.mode === 'stagehand') {
          // AI-Weg: oeffne das oberste Video mit dem Dubletten-Text, dann loeschen
          await ab.act(`open the first video thumbnail that shows the text "${MATCH}"`);
          ...
          await p.goto(`https://www.tiktok.com/@${PROFILE}`, { waitUntil: 'domcontentloaded', timeout: 45000 }); await sleep(5000);
        } else {
    ...
    await p.goto(`https://www.tiktok.com/@${PROFILE}`, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch(() => {});
    await sleep(4000); await p.screenshot({ path: path.join(SHOT, 'profile-after.png') });
    try { fs.writeFileSync(path.join(ROOT, 'reports', 'tiktok-delete.json'), JSON.stringify({ ts: new Date().toISOString(), mode: ab.mode, match: MATCH, deleted, dry: !GO }, null, 2)); } catch {}
