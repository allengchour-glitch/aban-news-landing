...
if (Test-Path $out) {
  ...
  git commit -m "aurora-video 1-Klick: drehendes Aurora-Video" 2>$null
  git pull --rebase origin claude/luxestyle-product-CizQ6 2>$null
  git push origin claude/luxestyle-product-CizQ6 2>$null
  Write-Host ""
  Write-Host "ERLEDIGT. Schreib Claude:  video da"
} else {
  Write-Host ""
  Write-Host "KEIN Video erstellt. Ursache: Gemini-Key ohne Billing-Projekt ODER Veo/fal-Fehler."
  Write-Host "Plan B: in der Gemini-App selbst generieren und Claude schicken."
}
