# worker-autodeploy.ps1 - LuxeStyle: Cloudflare-Worker SELBST deployen, wenn sich der Code geaendert hat.
# User 2026-06-19 "maximum auto": kein Klick mehr. Laeuft im Tages-Zyklus mit; deployt NUR bei Aenderung
# (Hash-Waechter) -> billig + selbstheilend. Voraussetzung: am PC 1 `wrangler login` (OAuth bleibt gespeichert)
...
# Hash ueber die deploy-relevanten Dateien (Inhalt = was live geht). queue.json liest der Worker live -> egal.
...
if ($cur.Trim() -eq $last.Trim()) { Write-Host "Worker unveraendert -> kein Deploy noetig."; exit 0 }
...
# Auth pruefen (OAuth-Login ODER API-Token)
$hasAuth = $false
...
if (-not $hasAuth) { Write-Host " Kein Cloudflare-Login. Am PC 1x `wrangler login` ODER CLOUDFLARE_API_TOKEN in luxe-secrets.ps1. Deploy uebersprungen."; exit 0 }

Set-Location $wdir
Write-Host "-> Worker-Code geaendert -> deploye..."
...
if ($LASTEXITCODE -eq 0) {
  ...
  Write-Host " Deploy fehlgeschlagen (Exit $LASTEXITCODE) - Stand NICHT gemerkt, probiert naechsten Lauf wieder."
}
