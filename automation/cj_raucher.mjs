...
const TERMS=['herb grinder','weed grinder','rolling tray','tobacco rolling machine','cigarette roller machine','smoking pipe','tobacco pipe','glass pipe','metal cigarette case','cigarette ashtray','hookah shisha',
 // fourtwenty-Sortiment: Papers, Filter-Tips, Cones, Bong, Stash, Dab
 'rolling papers','pre rolled cones','cigarette filter tips','activated carbon filter tips','glass water bong','dab rig','herb stash jar','smell proof bag'];
// BAN: alles was KEIN Raucher-Zeug ist, aber „grinder/mill/pipe/tray/filter/cone" im Namen trägt (Küche/Haustier/Werkzeug/Sanitär/Aquarium/HVAC/Deko).
const BAN=/wholesale|\bsample\b|replacement|kinder|for kids|baby|cbd|thc|weed leaf|vape|e-liquid|nicotine|nail|paw|claw|\bpet\b|\bdog\b|\bcat\b|foot|callus|meat|coffee|\bsalt\b|pepper|spice|garlic|angle grinder|\bwood\b|granite|drill|chuck|blender|mincer|kitchen|\bmill\b|grain|flour|bench grinder|sander|polish|whetstone|sharpen|tool set|cutting disc|pvc|plumb|drain|garden|hose|nut|aquarium|fish tank|hvac|air.?puri|purifier|composter|kompost|range.?hood|faucet|water.?filter|shower|humidifier|\bmask\b|vacuum|membrane|traffic|ice.?cream|speaker|\bfan\b|extractor|ventilation|oil diffuser|reusable straw|water bottle|tea |coffee/i;
// Positiv-Filter: Name MUSS eine raucher-spezifische Phrase enthalten (nicht bloss „grinder"/„pipe"/„filter"/„cone").
const SMOKE=/herb grinder|weed grinder|tobacco grinder|rolling tray|rolling paper|rolling machine|smoking pipe|tobacco pipe|glass pipe|hand pipe|water pipe|bubbler|hookah|shisha|ashtray|cigarette case|cigarette holder|cigar case|snuff|filter tips?|pre.?rolled|rolling cone|\bbong\b|dab rig|nectar collector|stash jar|smell.?proof|grinder card/i;
...
for(const term of TERMS){
  ...
  for(let page=1;page<=3 && total<CAP;page++){
    ...
    for(const p of list){
      ...
      const pr=parseFloat((''+p.sellPrice).split('--')[0])||0; if(pr<1||pr>90)continue;
