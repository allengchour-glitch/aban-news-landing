...
async function gemini(title,facts){
 const prompt=`Du bist Produkttexter für einen Schweizer Premium-Online-Shop (Stil wie Galaxus: sachlich, informativ, ausführlich, vertrauenswürdig). Schreibe eine DEUTSCHE Produktbeschreibung (220-320 Wörter) NUR auf Basis dieser Fakten – NICHTS erfinden, keine erfundenen Zahlen/Materialien:
Titel: ${title}
Fakten: ${facts||'(keine Detaildaten – halte dich allgemein und ehrlich)'}
HTML-Struktur: 1 Einleitungs-<p>, dann <h3>Das zeichnet es aus</h3> (2 <p>), <h3>Für wen geeignet</h3> (1 <p>), <h3>Gut zu wissen</h3> (1 <p>). Schweizer Rechtschreibung (ss statt ß). Kein Preis, kein Versand, kein Rabattcode. Gib NUR das HTML aus, ohne Markdown-Fences.`;
 for(let i=0;i<4;i++){
  const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${KEY}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.7,maxOutputTokens:900}})});
...
outer: while(true){
 ...
 for(const p of conn.nodes){
  ...
  const prose=await gemini(p.title,facts); await sleep(4200); // Rate-Limit Gemini free ~15rpm
