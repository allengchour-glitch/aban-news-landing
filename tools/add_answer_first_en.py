#!/usr/bin/env python3
# Answer-First ("In short:") auf englische Guide-Seiten (en/*.html ohne Tool-UI).
# Surfacet die echte erste FAQ-Antwort vor dem ersten <h2>. Idempotent.
import re, glob, os, html as _html
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
EN = os.path.join(ROOT, "en")
MARK = "data-aban-answer"
TOOL_RE = re.compile(r"(rechner|generator|checker|detektor|studio|werkzeug|calculator|bingo|prompt-check)", re.I)
BOX = ('<p class="answer-first" {m} style="background:#fff7ed;border:1px solid #fed7aa;'
       ...
       'font-size:1.02rem"><strong>In short:</strong> {a}</p>\n')
